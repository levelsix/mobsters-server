/**
 * This class is generated by jOOQ
 */
package com.lvl6.mobsters.db.jooq.generated.tables.pojos;


import com.lvl6.mobsters.db.jooq.generated.tables.interfaces.ISalesDisplayItemConfig;

import javax.annotation.Generated;
import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.Id;
import javax.persistence.Table;
import javax.validation.constraints.NotNull;


/**
 * This class is generated by jOOQ.
 */
@Generated(
	value = {
		"http://www.jooq.org",
		"jOOQ version:3.6.1"
	},
	comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
@Entity
@Table(name = "sales_display_item_config", schema = "mobsters")
public class SalesDisplayItemConfigPojo implements ISalesDisplayItemConfig {

	private static final long serialVersionUID = -211356911;

	private Integer id;
	private Integer salesPackageId;
	private Integer rewardId;

	public SalesDisplayItemConfigPojo() {}

	public SalesDisplayItemConfigPojo(SalesDisplayItemConfigPojo value) {
		this.id = value.id;
		this.salesPackageId = value.salesPackageId;
		this.rewardId = value.rewardId;
	}

	public SalesDisplayItemConfigPojo(
		Integer id,
		Integer salesPackageId,
		Integer rewardId
	) {
		this.id = id;
		this.salesPackageId = salesPackageId;
		this.rewardId = rewardId;
	}

	@Id
	@Column(name = "id", unique = true, nullable = false, precision = 10)
	@NotNull
	@Override
	public Integer getId() {
		return this.id;
	}

	@Override
	public SalesDisplayItemConfigPojo setId(Integer id) {
		this.id = id;
		return this;
	}

	@Column(name = "sales_package_id", precision = 10)
	@Override
	public Integer getSalesPackageId() {
		return this.salesPackageId;
	}

	@Override
	public SalesDisplayItemConfigPojo setSalesPackageId(Integer salesPackageId) {
		this.salesPackageId = salesPackageId;
		return this;
	}

	@Column(name = "reward_id", precision = 10)
	@Override
	public Integer getRewardId() {
		return this.rewardId;
	}

	@Override
	public SalesDisplayItemConfigPojo setRewardId(Integer rewardId) {
		this.rewardId = rewardId;
		return this;
	}

	// -------------------------------------------------------------------------
	// FROM and INTO
	// -------------------------------------------------------------------------

	/**
	 * {@inheritDoc}
	 */
	@Override
	public void from(ISalesDisplayItemConfig from) {
		setId(from.getId());
		setSalesPackageId(from.getSalesPackageId());
		setRewardId(from.getRewardId());
	}

	/**
	 * {@inheritDoc}
	 */
	@Override
	public <E extends ISalesDisplayItemConfig> E into(E into) {
		into.from(this);
		return into;
	}


	public String toString() {
		com.lvl6.mobsters.db.jooq.generated.tables.records.SalesDisplayItemConfigRecord poop = new com.lvl6.mobsters.db.jooq.generated.tables.records.SalesDisplayItemConfigRecord();
		poop.from(this);
		return "SalesDisplayItemConfigPojo[" + poop.valuesRow() + "]";
	}
}
