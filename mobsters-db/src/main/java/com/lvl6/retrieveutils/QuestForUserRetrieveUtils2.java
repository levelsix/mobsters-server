...
public class QuestForUserRetrieveUtils2 {
	...
	public List<QuestForUser> getUnredeemedIncompleteUserQuests(String userId) {
		...
		try {
			...
			String query = String.format(
					...
					DBConstants.QUEST_FOR_USER__USER_ID,
					DBConstants.QUEST_FOR_USER__IS_REDEEMED,
					DBConstants.QUEST_FOR_USER__IS_COMPLETE);

			userQuests = this.jdbcTemplate.query(query, values, rowMapper);
	...
	public Map<Integer, QuestForUser> getQuestIdToUnredeemedUserQuests(
			String userId) {
		log.debug("retrieving unredeemed user quests map for userId " + userId);

		Map<Integer, QuestForUser> questIdsToUnredeemedUserQuests = new HashMap<Integer, QuestForUser>();
	...
	public QuestForUser getSpecificUnredeemedUserQuest(String userId,
			int questId) {
		log.debug("retrieving specific unredeemed user quest for userid "
				+ userId + " and questId " + questId);
		...
		try {
			...
			List<QuestForUser> qfuList = this.jdbcTemplate.query(query, values,
					rowMapper);
	...
	private static final class UserQuestForClientMapper implements
			RowMapper<QuestForUser> {

		private static List<String> columnsSelected;

		@Override
		public QuestForUser mapRow(ResultSet rs, int rowNum)
				throws SQLException {
			...
			qfu.setRedeemed(rs
					.getBoolean(DBConstants.QUEST_FOR_USER__IS_REDEEMED));
			qfu.setComplete(rs
					.getBoolean(DBConstants.QUEST_FOR_USER__IS_COMPLETE));
			return qfu;
		}
