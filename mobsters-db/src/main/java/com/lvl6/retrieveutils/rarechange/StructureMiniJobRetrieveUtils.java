...
public class StructureMiniJobRetrieveUtils {
	...
	public StructureMiniJob getPredecessorMiniJobForStructId(int structId) {
		...
		if (null != curStruct) {
			...
			StructureMiniJob predecessorStruct = structIdsToMiniJobs
	...
	private StructureMiniJob convertRSRowToMiniJob(ResultSet rs)
		...
		int generatedJobLimit = rs
				.getInt(DBConstants.STRUCTURE_MINI_JOB__GENERATED_JOB_LIMIT);
		int hoursBetweenJobGeneration = rs
				.getInt(DBConstants.STRUCTURE_MINI_JOB__HOURS_BETWEEN_JOB_GENERATION);

		return new StructureMiniJob(structId, generatedJobLimit,
