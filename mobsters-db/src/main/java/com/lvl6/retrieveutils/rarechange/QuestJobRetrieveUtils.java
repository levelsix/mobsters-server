...
public class QuestJobRetrieveUtils {

	private static Logger log = LoggerFactory.getLogger(QuestJobRetrieveUtils.class);
	...
	public List<Integer> getQuestJobIdsForQuestIds(
		...
		for (Integer questId : questIdList) {
			Map<Integer, QuestJob> questJobIdsToQuestJobs = getQuestJobsForQuestId(questId);
	...
	private void setStaticQuestJobIdsToQuestJobsHelper(Connection conn,
		...
		try {
			...
			Map<Integer, Map<Integer, QuestJob>> questIdMapTmp = new HashMap<Integer, Map<Integer, QuestJob>>();
			Map<Integer, Map<Integer, QuestJob>> cityIdMapTmp = new HashMap<Integer, Map<Integer, QuestJob>>();
			...
			while (rs.next()) {
				...
				if (!questIdMapTmp.containsKey(questId)) {
					...
					questIdMapTmp
							.put(questId, new HashMap<Integer, QuestJob>());
				}
				...
				Map<Integer, QuestJob> questJobIdToQuestJobForQuestId = questIdMapTmp
						.get(questId);
				questJobIdToQuestJobForQuestId.put(questJobId, questJob);

				//id to quest job for cityId
				Map<Integer, QuestJob> questJobIdToQuestJobForCityId = cityIdMapTmp
						.get(cityId);
	...
	private QuestJob convertRSRowToQuestJob(ResultSet rs)
		...
		String questJobType = rs
				.getString(DBConstants.QUEST_JOB__QUEST_JOB_TYPE);
		...
		if (null != questJobType) {
			...
			if (!questJobType.equals(newQuestJobType)) {
				log.error(String.format("questJobType incorrect: %s, id=%s",
						questJobType, id));
