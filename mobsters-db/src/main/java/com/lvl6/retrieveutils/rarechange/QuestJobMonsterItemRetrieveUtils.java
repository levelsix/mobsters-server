...
public class QuestJobMonsterItemRetrieveUtils {

	private static Logger log = LoggerFactory.getLogger(QuestJobMonsterItemRetrieveUtils.class);
	...
	public QuestJobMonsterItem getItemForQuestJobAndMonsterId(
		...
		Map<Integer, QuestJobMonsterItem> monsterIdsToItems = questJobIdsToMonsterIdsToItems
				.get(questJobId);
		...
		if (monsterIdsToItems.containsKey(monsterId)) {
			...
			log.warn("no items for questJobId=" + questJobId + " monsterId="
					+ monsterId);
		}
		return null;
	...
	private void setStaticQuestJobIdsToMonsterIdsToItemsHelper(
		...
		try {
			rs.last();
			rs.beforeFirst();
			Map<Integer, Map<Integer, QuestJobMonsterItem>> questJobIdsToMonsterIdsToItemsTemp = new HashMap<Integer, Map<Integer, QuestJobMonsterItem>>();
			...
			while (rs.next()) {
				...
				Map<Integer, QuestJobMonsterItem> monsterIdsToItems = questJobIdsToMonsterIdsToItemsTemp
						.get(questJobId);
	...
	private QuestJobMonsterItem convertRSRowToQuestMonsterItem(
		...
		int monsterId = rs
				.getInt(DBConstants.QUEST_JOB_MONSTER_ITEM__MONSTER_ID);
		int itemId = rs.getInt(DBConstants.QUEST_JOB_MONSTER_ITEM__ITEM_ID);
		float itemDropRate = rs
				.getFloat(DBConstants.QUEST_JOB_MONSTER_ITEM__ITEM_DROP_RATE);
