...
public class PrerequisiteRetrieveUtils {

	private static Logger log = LoggerFactory.getLogger(PrerequisiteRetrieveUtils.class);
	...
	*/
			...
			if (conn != null) {
				...
				if (rs != null) {
					try {
						rs.last();
						rs.beforeFirst();
						Map<Integer, Prerequisite> prerequisiteIdsToPrerequisitesTemp = new HashMap<Integer, Prerequisite>();
						...
						while (rs.next()) {
							...
							prerequisiteIdsToPrerequisitesTemp.put(
	...
	private Prerequisite convertRSRowToPrerequisite(ResultSet rs)
		...
		String prereqGameType = rs
				.getString(DBConstants.PREREQUISITE__PREREQ_GAME_TYPE);
		int prereqGameEntityId = rs
				.getInt(DBConstants.PREREQUISITE__PREREQ_GAME_ENTITY_ID);
		...
		if (null != gameType) {
			...
			if (!gameType.equals(newGameType)) {
				log.error(String.format("gameType incorrect: %s, id=%s",
						gameType, id));
		...
		if (null != prereqGameType) {
			...
			if (!prereqGameType.equals(newPrereqGameType)) {
				log.error(String.format("prereqGameType incorrect: %s, id=%s",
						prereqGameType, id));
				prereqGameType = newPrereqGameType;
			}
		}

		Prerequisite prerequisite = new Prerequisite(id, gameType,
				gameEntityId, prereqGameType, prereqGameEntityId, quantity);
		return prerequisite;
	}
