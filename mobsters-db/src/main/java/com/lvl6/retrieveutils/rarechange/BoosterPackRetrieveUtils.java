...
public class BoosterPackRetrieveUtils {
				...
				if (rs != null) {
					try {
						...
						while (rs.next()) {  //should only be one
							...
							if (boosterPack != null)
								boosterPackIdsToBoosterPacksTemp.put(
										boosterPack.getId(), boosterPack);
						}
	...
	private BoosterPack convertRSRowToBoosterPack(ResultSet rs)
		...
		String listBackgroundImgName = rs
				.getString(DBConstants.BOOSTER_PACK__LIST_BACKGROUND_IMG_NAME);
		String listDescription = rs
				.getString(DBConstants.BOOSTER_PACK__LIST_DESCRIPTION);
		String navBarImgName = rs
				.getString(DBConstants.BOOSTER_PACK__NAV_BAR_IMG_NAME);
		String navTitleImgName = rs
				.getString(DBConstants.BOOSTER_PACK__NAV_TITLE_IMG_NAME);
		String machineImgName = rs
				.getString(DBConstants.BOOSTER_PACK__MACHINE_IMG_NAME);
		int expPerItem = 0;
		try {
		...
		boolean displayToUser = rs
				.getBoolean(DBConstants.BOOSTER_PACK__DISPLAY_TO_USER);
		...
		if (null != boosterPackType) {
			...
			if (!boosterPackType.equals(boosterPackType)) {
				log.error(String.format("boosterPackType incorrect: %s, id=%s",
						boosterPackType, id));
