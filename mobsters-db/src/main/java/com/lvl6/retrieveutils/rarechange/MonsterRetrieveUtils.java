...
public class MonsterRetrieveUtils {
				...
				if (rs != null) {
					try {
						...
						while (rs.next()) {  //should only be one
							...
							if (monster != null)
								monsterIdsToMonstersTemp.put(monster.getId(),
										monster);
						}
	...
	private Monster convertRSRowToMonster(ResultSet rs)
		...
		String evolutionGroup = rs
				.getString(DBConstants.MONSTER__EVOLUTION_GROUP);
		...
		int minutesToCombinePieces = rs
				.getInt(DBConstants.MONSTER__MINUTES_TO_COMBINE_PIECES);
		int maxLevel = rs.getInt(DBConstants.MONSTER__MAX_LEVEL);

		int evolutionMonsterId = rs
				.getInt(DBConstants.MONSTER__EVOLUTION_MONSTER_ID);
		int evolutionCatalystMonsterId = rs
				.getInt(DBConstants.MONSTER__EVOLUTION_CATALYST_MONSTER_ID);
		int minutesToEvolve = rs.getInt(DBConstants.MONSTER__MINUTES_TO_EVOLVE);
		int numCatalystsRequired = rs
				.getInt(DBConstants.MONSTER__NUM_EVOLUTION_CATALYSTS);

		String carrotRecruited = rs
				.getString(DBConstants.MONSTER__CARROT_RECRUITED);
		String carrotDefeated = rs
				.getString(DBConstants.MONSTER__CARROT_DEFEATED);
		String carrotEvolved = rs
				.getString(DBConstants.MONSTER__CARROT_EVOLVED);
		...
		String animationType = rs
				.getString(DBConstants.MONSTER__ANIMATION_TYPE);
		int verticalPixelOffset = rs
				.getInt(DBConstants.MONSTER__VERTICAL_PIXEL_OFFSET);
		String atkSoundFile = rs.getString(DBConstants.MONSTER__ATK_SOUND_FILE);
		int atkSoundAnimationFrame = rs
				.getInt(DBConstants.MONSTER__ATK_SOUND_ANIMATION_FRAME);
		int atkAnimationRepeatedFramesStart = rs
				.getInt(DBConstants.MONSTER__ATK_ANIMATION_REPEATED_FRAMES_START);
		int atkAnimationRepeatedFramesEnd = rs
				.getInt(DBConstants.MONSTER__ATK_ANIMATION_REPEATED_FRAMES_END);
		String shorterName = rs.getString(DBConstants.MONSTER__SHORT_NAME);
		float shadowScaleFactor = rs
				.getFloat(DBConstants.MONSTER__SHADOW_SCALE_FACTOR);
		int baseOffensiveSkillId = rs
				.getInt(DBConstants.MONSTER__BASE_OFFENSIVE_SKILL_ID);
		int baseDefensiveSkillId = rs
				.getInt(DBConstants.MONSTER__BASE_DEFENSIVE_SKILL_ID);
		int pvpMonsterDropId = rs
				.getInt(DBConstants.MONSTER__PVP_MONSTER_DROP_ID);

		if (null != quality) {
			String newQuality = quality.trim().toUpperCase();
			if (!quality.equals(newQuality)) {
				log.error(String.format("incorrect monster quality, %s, id=%s",
						quality, id));
				quality = newQuality;
			}
		}
		...
		if (null != element) {
			...
			if (!element.equals(newElement)) {
				log.error(String.format("incorrect monster element, %s, id=%s",
						element, id));
				element = newElement;
			}
		}
		...
		Monster monster = new Monster(id, evolutionGroup, monsterGroup,
				quality, evolutionLevel, displayName, element, imagePrefix,
				numPuzzlePieces, minutesToCombinePieces, maxLevel,
				evolutionMonsterId, evolutionCatalystMonsterId,
				minutesToEvolve, numCatalystsRequired, carrotRecruited,
				carrotDefeated, carrotEvolved, description, evolutionCost,
				animationType, verticalPixelOffset, atkSoundFile,
