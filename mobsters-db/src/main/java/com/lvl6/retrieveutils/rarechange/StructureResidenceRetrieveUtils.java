...
public class StructureResidenceRetrieveUtils {

	private static Logger log = LoggerFactory.getLogger(StructureResidenceRetrieveUtils.class);
	...
	public StructureResidence getUpgradedResidenceForStructId(
		...
		if (null != curStruct) {
			...
			StructureResidence upgradedStruct = structIdsToResidences
	...
	public StructureResidence getPredecessorResidenceForStructId(
		...
		if (null != curStruct) {
			...
			StructureResidence predecessorStruct = structIdsToResidences
	...
	private StructureResidence convertRSRowToResidence(ResultSet rs)
		...
		int numMonsterSlots = rs
				.getInt(DBConstants.STRUCTURE_RESIDENCE__NUM_MONSTER_SLOTS);
		int numBonusMonsterSlots = rs
				.getInt(DBConstants.STRUCTURE_RESIDENCE__NUM_BONUS_MONSTER_SLOTS);
		int numGemsRequired = rs
				.getInt(DBConstants.STRUCTURE_RESIDENCE__NUM_GEMS_REQUIRED);
		int numAcceptedFbInvites = rs
				.getInt(DBConstants.STRUCTURE_RESIDENCE__NUM_ACCEPETED_FB_INVITES);
		String occupationName = rs
				.getString(DBConstants.STRUCTURE_RESIDENCE__OCCUPATION_NAME);
		String imgSuffix = rs
				.getString(DBConstants.STRUCTURE_RESIDENCE__IMG_SUFFIX);

		return new StructureResidence(structId, numMonsterSlots,
				numBonusMonsterSlots, numGemsRequired, numAcceptedFbInvites,
				occupationName, imgSuffix);
	}
}
