...
public class CepfuRaidHistoryRetrieveUtils2 {

	private static Logger log = LoggerFactory.getLogger(CepfuRaidHistoryRetrieveUtils2.class);
	...
	public Map<Date, Map<String, CepfuRaidHistory>> getRaidHistoryForPastNDaysForClan(
		...
		try {
			List<CepfuRaidHistory> history = this.jdbcTemplate.query(query,
			...
			for (CepfuRaidHistory cepfurh : history) {
				...
				if (!timesToUserIdsToRaidHistory.containsKey(aDate)) {
					timesToUserIdsToRaidHistory.put(aDate,
							new HashMap<String, CepfuRaidHistory>());
				}
				Map<String, CepfuRaidHistory> userIdToRaidHistory = timesToUserIdsToRaidHistory
						.get(aDate);
			...
			log.info(String
					.format("getting raid history for past days. clanId=%s, pastDate=%s",
							clanId, pastDate));

		} catch (Exception e) {
			log.error(
					"clan event persistent for user raid history retrieve db error.",
					e);
			timesToUserIdsToRaidHistory = new HashMap<Date, Map<String, CepfuRaidHistory>>();
			//		} finally {
	...
	private static final class CepfuRaidHistoryForClientMapper implements
			RowMapper<CepfuRaidHistory> {

		private static List<String> columnsSelected;

		@Override
		public CepfuRaidHistory mapRow(ResultSet rs, int rowNum)
				throws SQLException {
			CepfuRaidHistory crh = new CepfuRaidHistory();
			crh.setUserId(rs.getString(DBConstants.CEPFU_RAID_HISTORY__USER_ID));
			Timestamp time = rs
					.getTimestamp(DBConstants.CEPFU_RAID_HISTORY__TIME_OF_ENTRY);
			if (null != time) {
			...
			crh.setCrDmgDone(rs
					.getInt(DBConstants.CEPFU_RAID_HISTORY__CR_DMG_DONE));
			crh.setClanCrDmg(rs
					.getInt(DBConstants.CEPFU_RAID_HISTORY__CLAN_CR_DMG));
		...
		public static List<String> getColumnsSelected() {
			if (null == columnsSelected) {
				...
				columnsSelected
						.add(DBConstants.CEPFU_RAID_HISTORY__TIME_OF_ENTRY);
				...
				columnsSelected
						.add(DBConstants.CEPFU_RAID_HISTORY__CR_DMG_DONE);
				columnsSelected
						.add(DBConstants.CEPFU_RAID_HISTORY__CLAN_CR_DMG);
