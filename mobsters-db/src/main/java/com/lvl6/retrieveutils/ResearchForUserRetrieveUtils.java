...
public class ResearchForUserRetrieveUtils {
	...
	public List<ResearchForUser> getAllResearchForUser(String userId) {
		...
		try {
			userResearch = this.jdbcTemplate.query(query, values, rowMapper);
		} catch (Exception e) {
	...
	public ResearchForUser getResearchForUser(String userResearchUuid) {
		log.debug("retrieving researchForUser for userResearchUuid {}",
				userResearchUuid);
	...
	private static final class UserResearchForClientMapper implements
			RowMapper<ResearchForUser> {

		private static List<String> columnsSelected;

		@Override
		public ResearchForUser mapRow(ResultSet rs, int rowNum)
				throws SQLException {
			...
			rfu.setResearchId(rs
					.getInt(DBConstants.RESEARCH_FOR_USER__RESEARCH_ID));
			rfu.setTimePurchased(rs
					.getTimestamp(DBConstants.RESEARCH_FOR_USER__TIME_PURCHASED));
			rfu.setComplete(rs
					.getBoolean(DBConstants.RESEARCH_FOR_USER__IS_COMPLETE));
		...
		public static List<String> getColumnsSelected() {
			if (null == columnsSelected) {
				...
				columnsSelected
						.add(DBConstants.RESEARCH_FOR_USER__TIME_PURCHASED);
