...
public class MonsterForUserRetrieveUtils2 {
	...
	public List<MonsterForUser> getMonstersForUser(String userId) {
		...
		try {
			userMonsters = this.jdbcTemplate.query(query, values, rowMapper);
		} catch (Exception e) {
	...
	public Map<String, List<MonsterForUser>> getUserIdsToMonsterTeamForUserIds(
		...
		log.info(String.format("RETRIEVING USERS' TEAMS. query=%s, values=%s",
				query, values));

		Map<String, List<MonsterForUser>> userIdsToCurrentTeam = new HashMap<String, List<MonsterForUser>>();
		try {
			List<MonsterForUser> mfuList = this.jdbcTemplate.query(query,
			...
			for (MonsterForUser userMonster : mfuList) {
				...
				List<MonsterForUser> currentTeam = userIdsToCurrentTeam
						.get(userId);
	...
	public MonsterForUser getSpecificUserMonster(String userMonsterId) {
		log.debug(String.format("retrieving user monster for userMonsterId=%s",
				userMonsterId));

		Object[] values = { userMonsterId };
		String query = String.format("select * from %s where %s=?", TABLE_NAME,
				DBConstants.MONSTER_FOR_USER__ID);
	...
	public Map<String, MonsterForUser> getSpecificUserMonsters(
			List<String> userMonsterIds) {
		log.debug("retrieving user monsters for userMonsterIds: "
				+ userMonsterIds);
	...
	public Map<String, MonsterForUser> getSpecificOrAllUserMonstersForUser(
		...
		if (userMonsterIds != null && !userMonsterIds.isEmpty()) {
			log.debug("retrieving user monster for userMonsterIds: "
					+ userMonsterIds);
			querySb.append(" AND ");
			querySb.append(DBConstants.MONSTER_FOR_USER__ID);
			querySb.append(" IN (");
	...
	public Map<Integer, MonsterForUser> getPieceDeficientIncompleteMonstersWithUserAndMonsterIds(
		...
		String query = String
				.format("select * from %s where %s=? and %s in (%s) and %s=? and %s=?;",
		...
		try {
			...
			for (MonsterForUser userMonster : mfuList) {
				monsterIdsToMonsters.put(userMonster.getMonsterId(),
						userMonster);
			}
	...
	public Map<String, Map<String, MonsterForUser>> getCompleteMonstersForUser(
		...
		Map<String, Map<String, MonsterForUser>> userIdsToMfuIdsToMonsters = new HashMap<String, Map<String, MonsterForUser>>();
		try {
			List<MonsterForUser> mfuList = this.jdbcTemplate.query(query,
					values.toArray(), rowMapper);
	...
	private static final class UserMonsterForClientMapper implements
			RowMapper<MonsterForUser> {

		private static List<String> columnsSelected;

		@Override
		public MonsterForUser mapRow(ResultSet rs, int rowNum)
				throws SQLException {
			...
			mfu.setMonsterId(rs
					.getInt(DBConstants.MONSTER_FOR_USER__MONSTER_ID));
			mfu.setCurrentExp(rs
					.getInt(DBConstants.MONSTER_FOR_USER__CURRENT_EXPERIENCE));
			mfu.setCurrentLvl(rs
					.getInt(DBConstants.MONSTER_FOR_USER__CURRENT_LEVEL));
			mfu.setCurrentHealth(rs
					.getInt(DBConstants.MONSTER_FOR_USER__CURRENT_HEALTH));
			mfu.setNumPieces(rs
					.getInt(DBConstants.MONSTER_FOR_USER__NUM_PIECES));
			mfu.setHasAllPieces(rs
					.getBoolean(DBConstants.MONSTER_FOR_USER__HAS_ALL_PIECES));
			mfu.setComplete(rs
					.getBoolean(DBConstants.MONSTER_FOR_USER__IS_COMPLETE));

			try {
				Timestamp time = rs
						.getTimestamp(DBConstants.MONSTER_FOR_USER__COMBINE_START_TIME);
			...
			mfu.setTeamSlotNum(rs
					.getInt(DBConstants.MONSTER_FOR_USER__TEAM_SLOT_NUM));
			mfu.setSourceOfPieces(rs
					.getString(DBConstants.MONSTER_FOR_USER__SOURCE_OF_PIECES));
			mfu.setRestricted(rs
					.getBoolean(DBConstants.MONSTER_FOR_USER__RESTRICTED));
			mfu.setOffensiveSkillId(rs
					.getInt(DBConstants.MONSTER_FOR_USER__OFFENSIVE_SKILL_ID));
			mfu.setDefensiveSkillId(rs
					.getInt(DBConstants.MONSTER_FOR_USER__DEFENSIVE_SKILL_ID));
		...
		public static List<String> getColumnsSelected() {
			if (null == columnsSelected) {
				...
				columnsSelected
						.add(DBConstants.MONSTER_FOR_USER__CURRENT_EXPERIENCE);
				columnsSelected
						.add(DBConstants.MONSTER_FOR_USER__CURRENT_LEVEL);
				columnsSelected
						.add(DBConstants.MONSTER_FOR_USER__CURRENT_HEALTH);
				...
				columnsSelected
						.add(DBConstants.MONSTER_FOR_USER__TEAM_SLOT_NUM);
				columnsSelected
						.add(DBConstants.MONSTER_FOR_USER__SOURCE_OF_PIECES);
				columnsSelected.add(DBConstants.MONSTER_FOR_USER__RESTRICTED);
				columnsSelected
						.add(DBConstants.MONSTER_FOR_USER__OFFENSIVE_SKILL_ID);
				columnsSelected
						.add(DBConstants.MONSTER_FOR_USER__DEFENSIVE_SKILL_ID);
			}
