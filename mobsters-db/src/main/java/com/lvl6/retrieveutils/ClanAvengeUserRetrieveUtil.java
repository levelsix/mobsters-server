...
public class ClanAvengeUserRetrieveUtil {
	private static Logger log = LoggerFactory
			.getLogger(ClanAvengeUserRetrieveUtil.class);
	...
	public List<ClanAvengeUser> getClanAvengeUser(String clanId) {
		...
		try {
			...
			if (null != clanId && !clanId.isEmpty()) {
				equalityConditions.put(DBConstants.CLAN_AVENGE_USER__CLAN_ID,
						clanId);
			}
			...
			String query = getQueryConstructionUtil()
					.selectRowsQueryEqualityConditions(columnsToSelected,
							...
							preparedStatement);
			log.info("getClanAvengeUser() query={} \t values={}", query, values);
			clanAvengeUsers = this.jdbcTemplate.query(query, values.toArray(),
					rowMapper);
	...
	public Map<String, List<ClanAvengeUser>> getClanAvengeUserMap(String clanId) {
		...
		Map<String, List<ClanAvengeUser>> clanAvengeIdToClanAvenge = new HashMap<String, List<ClanAvengeUser>>();
		for (ClanAvengeUser cau : allClanAvengeUser) {
			...
			List<ClanAvengeUser> usersForRetaliation = clanAvengeIdToClanAvenge
					.get(clanAvengeId);
	...
	private static final class ClanAvengeUserForClientMapper implements
			RowMapper<ClanAvengeUser> {

		private static List<String> columnsSelected;

		@Override
		public ClanAvengeUser mapRow(ResultSet rs, int rowNum)
				throws SQLException {
			ClanAvengeUser ca = new ClanAvengeUser();

			ca.setClanId(rs.getString(DBConstants.CLAN_AVENGE_USER__CLAN_ID));
			ca.setClanAvengeId(rs
					.getString(DBConstants.CLAN_AVENGE_USER__CLAN_AVENGE_ID));
			ca.setUserId(rs.getString(DBConstants.CLAN_AVENGE_USER__USER_ID));

			Timestamp ts = rs
					.getTimestamp(DBConstants.CLAN_AVENGE_USER__AVENGE_TIME);
			if (null != ts) {
		...
		public static List<String> getColumnsSelected() {
			if (null == columnsSelected) {
				...
				columnsSelected
						.add(DBConstants.CLAN_AVENGE_USER__CLAN_AVENGE_ID);
