...
public class PvpUserRetrieveUtil {
		...
		try {
			...
			String query = getQueryConstructionUtil()
					.selectRowsQueryEqualityConditions(columnsToSelect,
							TABLE_NAME, inConditions, conditionDelimiter,
							values, preparedStatement);

			log.info("query={}", query);

			List<PvpUser> puList = this.jdbcTemplate.query(query,
					new PvpUserMapper());
	 ...
	 * @param shieldEndTime Limits pvp users with actual and in battle shield
	 *            end times before this value.
	 * @param limit Limits amount of rows returned
	 * @param excludeUserIds When fetching rows, ignores rows with these ids.
	 * @return Returns pvp users whose elo is in the range (minElo, maxElo)
	 *         shieldEndTime < @param shieldEndTime and inBattleShieldEndTime < @param
	 *         shieldEndTime and userId not in @param excludeUserIds
	 */
	protected Set<PvpUser> retrievePvpUsers(int minElo, int maxElo,
		...
		try {
			...
			lessThanConditions
					.put(DBConstants.PVP_LEAGUE_FOR_USER__ELO, maxElo);
			...
			greaterThanConditions.put(DBConstants.PVP_LEAGUE_FOR_USER__ELO,
					minElo);
			...
			boolean preparedStatement = false;
			String query = getQueryConstructionUtil()
					.selectRowsQueryComparisonConditions(columnsToSelect,
							TABLE_NAME, equalityConditions, equalityCondDelim,
							lessThanConditions, lessThanCondDelim,
							greaterThanConditions, greaterThanCondDelim,
							delimAcrossConditions, values, preparedStatement, 0);
	...
	protected PvpUser getUserPvpLeagueForId(String userId) {
		...
		try {
			...
			pu = this.jdbcTemplate.queryForObject(query, values.toArray(),
					new PvpUserMapper());
		} catch (Exception e) {
	...
	public Map<String, PvpUser> getUserPvpLeagueForUsers(List<String> userIdList) {
		...
		try {
			...
			log.error("could not retrieve user pvpLeague for userIds="
					+ userIdList, e);
		}
