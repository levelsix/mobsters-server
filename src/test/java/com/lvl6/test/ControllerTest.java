...
public class ControllerTest extends TestCase {
	...
	public void testHazelcastPvpUtilQuery() {
		...
		try {
			predicate = getHazelcastPvpUtil().generatePredicate(minElo, maxElo,
	...
	*/
	...
	private Map<String, MonsterForUser> createCompleteMonsters(String userId,
		...
		List<Monster> twoMonsters = new ArrayList<Monster>(MonsterRetrieveUtils
				.getMonsterIdsToMonsters().values());
		...
		for (Monster monzter : twoMonsters) {
			...
			MonsterForUser newUserMonster = MonsterStuffUtils
					.createNewUserMonster(userId, monzter.getNumPuzzlePieces(),
							monzter, now, hasAllPieces, isComplete);
	...
	public void testBuyingMoneyTree() {
		User user = userRetrieveUtil
				.getUserById("0185e5f9-622a-415b-8444-d3743cbf8442");
	...
	private String insertIntoUserMonsterTable(String userId, int monsterId,
			int currExp, int currLvl, int currHealth, int numPieces,
			int isComplete, Timestamp combineStartTime, int teamSlotNum,
			String sourceOfPieces, int hasAllPieces, int restricted) {
		...
		insertParams.put(DBConstants.MONSTER_FOR_USER__CURRENT_EXPERIENCE,
				currExp);
		insertParams.put(DBConstants.MONSTER_FOR_USER__CURRENT_LEVEL, currLvl);
		insertParams.put(DBConstants.MONSTER_FOR_USER__CURRENT_HEALTH,
				currHealth);
		...
		insertParams.put(DBConstants.MONSTER_FOR_USER__COMBINE_START_TIME,
				combineStartTime);
		insertParams.put(DBConstants.MONSTER_FOR_USER__TEAM_SLOT_NUM,
				teamSlotNum);
		insertParams.put(DBConstants.MONSTER_FOR_USER__SOURCE_OF_PIECES,
				sourceOfPieces);
		insertParams.put(DBConstants.MONSTER_FOR_USER__HAS_ALL_PIECES,
				hasAllPieces);
	...
	public void testMonsterEnhancingAndHistoryTables() {
		////////////////////////ENHANCE REQUEST//////////////////////////////
		User user = userRetrieveUtil
				.getUserById("02ae9fb2-5117-4f18-b05c-de4b19a6aaad");
		int userOil = user.getOil();
		int monsterEnhanceRows1 = retrieveNumberOfMonsterEnhanceHistoryRows(user
				.getId());
		List<MonsterForUser> userMonsterList1 = monsterForUserRetrieveUtils
				.getMonstersForUser(user.getId());

		//give user the three necessary monsters used in evolving
		Date now = new Date();
		Timestamp nowTimestamp = new Timestamp(now.getTime());
		String userFeederMonsterId1 = insertIntoUserMonsterTable(user.getId(),
				1, 1000, 10, 10, 1, 1, nowTimestamp, 0,
				" cheater, cheater, pumpkin eater ", 1, 0);
		String userFeederMonsterId2 = insertIntoUserMonsterTable(user.getId(),
				1, 0, 1, 10, 1, 1, nowTimestamp, 0,
				" cheater, cheater, pumpkin eater ", 1, 0);
		String userBeingEnhancedMonsterId = insertIntoUserMonsterTable(
				user.getId(), 1750, 0, 1, 10, 1, 1, nowTimestamp, 0,
				" cheater, cheater, pumpkin eater ", 1, 0);

		MinimumUserProtoWithMaxResources.Builder mupWithMaxResources = MinimumUserProtoWithMaxResources
				.newBuilder();
		mupWithMaxResources.setMinUserProto(CreateInfoProtoUtils
				.createMinimumUserProtoFromUserAndClan(user, null));
		mupWithMaxResources.setMaxCash(1000000);
		mupWithMaxResources.setMaxOil(1000000);

		SubmitMonsterEnhancementRequestProto.Builder smerpb = SubmitMonsterEnhancementRequestProto
				.newBuilder();
		...
		UserEnhancementItemProto ueipEnhancedMonster = CreateInfoProtoUtils
		...
		UserEnhancementItemProto ueipFeeder1 = CreateInfoProtoUtils
				.createUserEnhancementItemProtoFromObj(mefu2);
		...
		UserEnhancementItemProto ueipFeeder2 = CreateInfoProtoUtils
				.createUserEnhancementItemProtoFromObj(mefu3);
		...
		User user2 = userRetrieveUtil
				.getUserById("02ae9fb2-5117-4f18-b05c-de4b19a6aaad");
		List<MonsterForUser> userMonsterList2 = monsterForUserRetrieveUtils
				.getMonstersForUser(user2.getId());
		int monsterDeleteHistoryRows2 = retrieveNumberOfMonsterDeleteHistoryRows(user2
				.getId());
		int monsterEnhanceRows2 = retrieveNumberOfMonsterEnhanceHistoryRows(user2
				.getId());
		...
		EnhancementWaitTimeCompleteRequestProto.Builder ewtcrpb = EnhancementWaitTimeCompleteRequestProto
				.newBuilder();
		ewtcrpb.setSender(CreateInfoProtoUtils
				.createMinimumUserProtoFromUserAndClan(user2, null));
		...
		Map<String, MonsterEnhancingForUser> enhancingMonstersMap = monsterEnhancingForUserRetrieveUtil
				.getMonstersForUser(user3.getId());
		MonsterEnhancingForUser mefuNonFeeder = enhancingMonstersMap
				.get(userBeingEnhancedMonsterId);
		...
		CollectMonsterEnhancementRequestProto.Builder cmerpb = CollectMonsterEnhancementRequestProto
				.newBuilder();
		cmerpb.setSender(CreateInfoProtoUtils
				.createMinimumUserProtoFromUserAndClan(user3, null));

		UserMonsterCurrentExpProto.Builder umcepb = UserMonsterCurrentExpProto
				.newBuilder();
		...
		User user4 = userRetrieveUtil
				.getUserById("02ae9fb2-5117-4f18-b05c-de4b19a6aaad");
		int monsterDeleteHistoryRows3 = retrieveNumberOfMonsterDeleteHistoryRows(user4
				.getId());
		List<MonsterForUser> userMonsterList3 = monsterForUserRetrieveUtils
				.getMonstersForUser(user4.getId());
		int monsterEnhanceRows3 = retrieveNumberOfMonsterEnhanceHistoryRows(user4
				.getId());
	...
	public void testMonsterEvolutionFinishedAndHistoryTables() {
		...
		User user = userRetrieveUtil
				.getUserById("02ae9fb2-5117-4f18-b05c-de4b19a6aaad");
		int userOil = user.getOil();
		int monsterEvolutionHistoryRows1 = retrieveNumberOfMonsterEvolutionHistoryRows(user
				.getId());
		List<MonsterForUser> userMonsterList1 = monsterForUserRetrieveUtils
				.getMonstersForUser(user.getId());
		int monsterDeleteHistoryRows1 = retrieveNumberOfMonsterDeleteHistoryRows(user
				.getId());

		//give user the three necessary monsters used in evolving
		Date now = new Date();
		Timestamp nowTimestamp = new Timestamp(now.getTime());
		String userMonsterId1 = insertIntoUserMonsterTable(user.getId(), 1,
				1000, 10, 10, 1, 1, nowTimestamp, 0,
				" cheater, cheater, pumpkin eater ", 1, 0);
		String userMonsterId2 = insertIntoUserMonsterTable(user.getId(), 1, 0,
				1, 10, 1, 1, nowTimestamp, 0,
				" cheater, cheater, pumpkin eater ", 1, 0);
		String userCatalystMonsterId = insertIntoUserMonsterTable(user.getId(),
				1750, 0, 1, 10, 1, 1, nowTimestamp, 0,
				" cheater, cheater, pumpkin eater ", 1, 0);

		EvolveMonsterRequestProto.Builder emrpb = EvolveMonsterRequestProto
				.newBuilder();
		emrpb.setSender(CreateInfoProtoUtils
				.createMinimumUserProtoFromUserAndClan(user, null));

		MonsterEvolvingForUser mefu = new MonsterEvolvingForUser(
				userCatalystMonsterId, userMonsterId1, userMonsterId2,
				user.getId(), nowTimestamp);
		UserMonsterEvolutionProto umep = CreateInfoProtoUtils
		...
		EvolutionFinishedRequestProto.Builder efrpb = EvolutionFinishedRequestProto
				.newBuilder();
		efrpb.setSender(CreateInfoProtoUtils
				.createMinimumUserProtoFromUserAndClan(user, null));
		...
		User user3 = userRetrieveUtil
				.getUserById("02ae9fb2-5117-4f18-b05c-de4b19a6aaad");
		int monsterDeleteHistoryRows2 = retrieveNumberOfMonsterDeleteHistoryRows(user3
				.getId());
		List<MonsterForUser> userMonsterList3 = monsterForUserRetrieveUtils
				.getMonstersForUser(user.getId());
		int monsterEvolutionHistoryRows2 = retrieveNumberOfMonsterEvolutionHistoryRows(user2
				.getId());

		assertEquals(userMonsterList2.size() - 2, userMonsterList3.size());
		assertEquals(monsterDeleteHistoryRows1 + 3, monsterDeleteHistoryRows2);
		assertEquals(monsterEvolutionHistoryRows1 + 1,
				monsterEvolutionHistoryRows2);
		...
		try {
			if (conn != null) {
				...
				absoluteConditionParams.put(
						DBConstants.MONSTER_EVOLVING_HISTORY__USER_ID, userId);
				rs = DBConnection.get().selectRowsAbsoluteAnd(conn,
						absoluteConditionParams,
						DBConstants.TABLE_MONSTER_EVOLVING_HISTORY);
				rs.last();
				size = rs.getRow();
			}
		} catch (Exception e) {
	...
	private int retrieveNumberOfMonsterEnhanceHistoryRows(String userId) {
		...
		try {
			if (conn != null) {
				Map<String, Object> absoluteConditionParams = new HashMap<String, Object>();
				absoluteConditionParams.put(
						DBConstants.MONSTER_ENHANCING_HISTORY__USER_ID, userId);
				rs = DBConnection.get().selectRowsAbsoluteAnd(conn,
						absoluteConditionParams,
						DBConstants.TABLE_MONSTER_ENHANCING_HISTORY);
				rs.last();
				size = rs.getRow();
			}
		} catch (Exception e) {
	...
	private int retrieveNumberOfMonsterDeleteHistoryRows(String userId) {
		...
		try {
			if (conn != null) {
				Map<String, Object> absoluteConditionParams = new HashMap<String, Object>();
				absoluteConditionParams.put(
						DBConstants.MONSTER_FOR_USER_DELETED__USER_ID, userId);
				rs = DBConnection.get().selectRowsAbsoluteAnd(conn,
						absoluteConditionParams,
						DBConstants.TABLE_MONSTER_FOR_USER_DELETED);
				rs.last();
				size = rs.getRow();
			}
		} catch (Exception e) {
	...
	public void testSellingMonsterAndHistory() {
		User user = userRetrieveUtil
				.getUserById("02ae9fb2-5117-4f18-b05c-de4b19a6aaad");
		List<MonsterForUser> userMonsterList = monsterForUserRetrieveUtils
				.getMonstersForUser(user.getId());
		int monsterDeleteHistoryRows = retrieveNumberOfMonsterDeleteHistoryRows(user
				.getId());
		int userCash = user.getCash();

		Date now = new Date();
		Timestamp nowTimestamp = new Timestamp(now.getTime());
		String userMonsterId = insertIntoUserMonsterTable(user.getId(), 1,
				1000, 10, 10, 1, 1, nowTimestamp, 0,
				" cheater, cheater, pumpkin eater ", 1, 0);
		...
		MinimumUserMonsterSellProto.Builder mumsp = MinimumUserMonsterSellProto
				.newBuilder();
		...
		SellUserMonsterRequestProto.Builder sumrpb = SellUserMonsterRequestProto
				.newBuilder();
	...
	public void testStaticDataContainer() {
		...
		assertNotNull("StaticDataContainer returns null StaticDataProto",
				staticData);
	...
	public void setUserCreateController(
			UserCreateController userCreateController) {
	...
	public void setRetrieveClanInfoController(
			RetrieveClanInfoController retrieveClanInfoController) {
