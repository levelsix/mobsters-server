...
public class GameEventSerializer extends AbstractByteArraySerializer {
	...
	private static final Logger log = LoggerFactory
			.getLogger(GameEventSerializer.class);

	/**
	 * This class serializes/deserializes GameEvents for Spring IP socket
	 * listeners
	 */
	...
	public byte[] deserialize(InputStream inputStream) throws IOException {
		...
		log.debug("Deserializing message...available to read:"
		...
		while (true) {
			...
			if (n < 12 && n > 7) {
				...
				if (n == 11) {
					payloadSize = ByteBuffer.wrap(paysize)
							.order(getByteOrder()).getInt()
			...
			if (n >= maxMessageSize) {
				throw new IOException(
						"event not ready before max message length: "
								+ maxMessageSize);
			}
			if (payloadSize > maxMessageSize) {
				throw new IOException("Message size: " + payloadSize
						+ " is greater then max message size: "
						+ maxMessageSize);
			}
			n++;
		}
