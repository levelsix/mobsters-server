...
public class PvpBattleHistoryRetrieveUtil2 {
	private static Logger log = LoggerFactory
			.getLogger(PvpBattleHistoryRetrieveUtil2.class);
	...
	private static final class PvpBattleHistoryForClientMapper implements
			RowMapper<PvpBattleHistory> {

		private static List<String> columnsSelected;

		@Override
		public PvpBattleHistory mapRow(ResultSet rs, int rowNum)
				throws SQLException {
			PvpBattleHistory history = new PvpBattleHistory();
			history.setAttackerId(rs
					.getString(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_ID));
			history.setDefenderId(rs
					.getString(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_ID));
			Timestamp time = rs
					.getTimestamp(DBConstants.PVP_BATTLE_HISTORY__BATTLE_END_TIME);
			...
			history.setAttackerPrevLeague(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_PREV_LEAGUE));
			history.setAttackerCurLeague(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_CUR_LEAGUE));
			history.setDefenderPrevLeague(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_PREV_LEAGUE));
			history.setDefenderCurLeague(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_CUR_LEAGUE));

			history.setAttackerPrevRank(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_PREV_RANK));
			history.setAttackerCurRank(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_CUR_RANK));
			history.setDefenderPrevRank(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_PREV_RANK));
			history.setDefenderCurRank(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_CUR_RANK));

			history.setDefenderCashChange(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_CASH_CHANGE));
			history.setDefenderOilChange(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_OIL_CHANGE));
			history.setAttackerCashChange(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_CASH_CHANGE));
			history.setAttackerOilChange(rs
					.getInt(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_OIL_CHANGE));
			history.setAttackerWon(rs
		...
		public static List<String> getColumnsSelected() {
			if (null == columnsSelected) {
				...
				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_ID);
				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_ID);
				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__BATTLE_END_TIME);

				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_PREV_LEAGUE);
				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_CUR_LEAGUE);
				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_PREV_LEAGUE);
				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_CUR_LEAGUE);

				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_PREV_RANK);
				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__ATTACKER_CUR_RANK);
				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_PREV_RANK);
				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_CUR_RANK);

				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_CASH_CHANGE);
				columnsSelected
						.add(DBConstants.PVP_BATTLE_HISTORY__DEFENDER_OIL_CHANGE);
				columnsSelected
	...
	public List<PvpBattleHistory> getRecentNBattlesForUserId(String userId,
			int n) {
		List<PvpBattleHistory> recentNBattles = null;
		...
		String query = String.format(
				...
				TABLE_NAME, DBConstants.PVP_BATTLE_HISTORY__DEFENDER_ID,
		...
		try {
			recentNBattles = this.jdbcTemplate.query(query, values, rowMapper);
		} catch (Exception e) {
			log.error(
					String.format(
							"error retrieving pvp_battle_history for userId=%s",
							userId), e);
			recentNBattles = new ArrayList<PvpBattleHistory>();
		}
		return recentNBattles;
	...
	public PvpBattleHistory getPvpBattle(String attackerId, String defenderId,
			Timestamp battleEndTime) {
		PvpBattleHistory battle = null;

		Object[] values = { attackerId, defenderId, battleEndTime, false };
		String query = String
				.format("select * from %s where %s in (?) and %s in (?) and %s=? and %s=?",
						TABLE_NAME,
						DBConstants.PVP_BATTLE_HISTORY__ATTACKER_ID,
		...
		log.info("query={}, values={}", query, values);
		try {
			List<PvpBattleHistory> battles = this.jdbcTemplate.query(query,
					values, rowMapper);
			...
			log.error(
					String.format(
							...
							new Object[] { attackerId, defenderId,
									battleEndTime }), e);
		}
		return battle;
	}
