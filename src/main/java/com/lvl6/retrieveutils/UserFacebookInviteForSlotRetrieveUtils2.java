...
@Component @DependsOn("gameServer") public class UserFacebookInviteForSlotRetrieveUtils2 {
	...
	public UserFacebookInviteForSlot getInviteForId(String inviteId) {
		...
		String query = String.format(
			...
			TABLE_NAME, DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__ID);

		UserFacebookInviteForSlot invite = null;
		try {
			List<UserFacebookInviteForSlot> inv = this.jdbcTemplate
...
//			DBConnection.get().close(rs, null, conn);
	...
	public Map<String, UserFacebookInviteForSlot> getInviteForId(List<String> inviteIds) {
		...
		try {
			List<UserFacebookInviteForSlot> invites = this.jdbcTemplate
...
//			DBConnection.get().close(rs, null, conn);
	...
	public Map<String, UserFacebookInviteForSlot> getSpecificOrAllInvitesForInviter(
		String userId, List<String> specificInviteIds, boolean filterByAccepted,
...
//			DBConnection.get().close(rs, null, conn);
	...
	public Map<String, UserFacebookInviteForSlot> getSpecificOrAllInvitesForRecipient(
		String recipientFacebookId, List<String> specificInviteIds, boolean filterByAccepted,
		boolean isAccepted, boolean filterByRedeemed, boolean isRedeemed) {
...
//			DBConnection.get().close(rs, null, conn);
	...
	private static final class UserFbInviteForClientMapper implements RowMapper<UserFacebookInviteForSlot> {

		private static List<String> columnsSelected;

		public UserFacebookInviteForSlot mapRow(ResultSet rs, int rowNum) throws SQLException {
			UserFacebookInviteForSlot invite = new UserFacebookInviteForSlot();
			invite.setId(rs.getString(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__ID));
			invite.setInviterUserId(rs.getString(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__INVITER_USER_ID));
			invite.setRecipientFacebookId(rs.getString(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__RECIPIENT_FACEBOOK_ID));
			
			try {
				Timestamp time = rs.getTimestamp(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__TIME_OF_INVITE);
			...
			try {
				Timestamp time = rs.getTimestamp(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__TIME_ACCEPTED);
				if (null != time && !rs.wasNull()) {
					Date date = new Date(time.getTime());
			...
			invite.setUserStructId(rs.getString(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__USER_STRUCT_ID));
			invite.setUserStructFbLvl(rs.getInt(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__USER_STRUCT_FB_LVL));

			try {
				Timestamp time = rs.getTimestamp(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__TIME_REDEEMED);
				if (null != time && !rs.wasNull()) {
					Date date = new Date(time.getTime());
		...
		public static List<String> getColumnsSelected() {
			if (null == columnsSelected) {
				...
				columnsSelected.add(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__ID);
				columnsSelected.add(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__INVITER_USER_ID);
				columnsSelected.add(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__RECIPIENT_FACEBOOK_ID);
				columnsSelected.add(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__TIME_OF_INVITE);
				columnsSelected.add(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__TIME_ACCEPTED);
				columnsSelected.add(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__USER_STRUCT_ID);
				columnsSelected.add(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__USER_STRUCT_FB_LVL);
				columnsSelected.add(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__TIME_REDEEMED);
			}
	...
	private static final class UserIdMapper implements RowMapper<String> {
		public String mapRow(ResultSet rs, int rowNum) throws SQLException {
			return rs.getString(DBConstants.USER_FACEBOOK_INVITE_FOR_SLOT__INVITER_USER_ID);
		}
