...
@Component @DependsOn("gameServer") public class MonsterHealingForUserRetrieveUtils2 {
	...
	public Map<String, MonsterHealingForUser> getMonstersForUser(String userId) {
		...
		String query = String.format(
			...
			TABLE_NAME, DBConstants.MONSTER_HEALING_FOR_USER__USER_ID);
		
		Map<String, MonsterHealingForUser> monsterIdsToMonsters =
			new HashMap<String, MonsterHealingForUser>();
		try {
...
//			DBConnection.get().close(rs, null, conn);
	...
	private static final class UserMonsterEvolvingForClientMapper implements RowMapper<MonsterHealingForUser> {
		...
		public MonsterHealingForUser mapRow(ResultSet rs, int rowNum) throws SQLException {
			MonsterHealingForUser mefu = new MonsterHealingForUser();

			mefu.setUserId(rs.getString(DBConstants.MONSTER_HEALING_FOR_USER__USER_ID));
			mefu.setMonsterForUserId(rs.getString(DBConstants.MONSTER_HEALING_FOR_USER__MONSTER_FOR_USER_ID));

			Timestamp time = rs.getTimestamp(DBConstants.MONSTER_HEALING_FOR_USER__QUEUED_TIME);
