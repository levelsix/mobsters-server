...
public class ClanHelpCountForUserRetrieveUtil {
	private static Logger log = LoggerFactory
			.getLogger(ClanHelpCountForUserRetrieveUtil.class);
	...
	public Map<String, UserClanHelpCount> getUserIdToClanHelpCountForClan(
		...
		try {
			...
			String query = String
					.format("SELECT %s, %s, sum(%s) as %s, sum(%s) as %s FROM %s WHERE %s > ? and %s=? GROUP BY %s",
			...
			log.info("getUserIdToClanHelpCountForClan() query={}, values={}",
					query, values);
			List<UserClanHelpCount> chList = this.jdbcTemplate.query(query,
	...
	public static class UserClanHelpCount {
		...
		public String toString() {
			return "UserClanHelpCount [userId=" + userId + ", clanId=" + clanId
					+ ", numSolicited=" + numSolicited + ", numGiven="
					+ numGiven + "]";
		}
	...
	private static final class UserClanHelpCountForClientMapper implements
			RowMapper<UserClanHelpCount> {

		private static List<String> columnsSelected;

		@Override
		public UserClanHelpCount mapRow(ResultSet rs, int rowNum)
				throws SQLException {
			UserClanHelpCount uchc = new UserClanHelpCount();
			uchc.setUserId(rs
					.getString(DBConstants.CLAN_HELP_COUNT_FOR_USER__USER_ID));
			uchc.setClanId(rs
					.getString(DBConstants.CLAN_HELP_COUNT_FOR_USER__CLAN_ID));
			uchc.setNumSolicited(rs
					.getInt(DBConstants.CLAN_HELP_COUNT_FOR_USER__SOLICITED));
			uchc.setNumGiven(rs
					.getInt(DBConstants.CLAN_HELP_COUNT_FOR_USER__GIVEN));
		...
		public static List<String> getColumnsSelected() {
			if (null == columnsSelected) {
				...
				columnsSelected
						.add(DBConstants.CLAN_HELP_COUNT_FOR_USER__USER_ID);
				columnsSelected
						.add(DBConstants.CLAN_HELP_COUNT_FOR_USER__CLAN_ID);
				columnsSelected
						.add(DBConstants.CLAN_HELP_COUNT_FOR_USER__SOLICITED);
				columnsSelected
						.add(DBConstants.CLAN_HELP_COUNT_FOR_USER__GIVEN);
			}
