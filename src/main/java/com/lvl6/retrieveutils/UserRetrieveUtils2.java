...
@Component @DependsOn("gameServer") public class UserRetrieveUtils2 {
	...
	public List<String> getUserIdsForFacebookIds(List<String> facebookIds) {
		...
		try {
			userIdList = this.jdbcTemplate
				.queryForList(query, params.toArray(), String.class);
		} catch (Exception e) {
...
//			DBConnection.get().close(rs, null, conn);
	...
	public Map<String, User> getUsersForFacebookIdsOrUserIds(List<String> facebookIds,
		List<String> userIds) {
		...
		try {
			List<User> usersList = this.jdbcTemplate
			...
			log.error("getUsersForFacebookIdsOrUserIds user retrieve db error.", e);
//		} finally {
//			DBConnection.get().close(rs, null, conn);
		}
		return userMap;
	...
	public User getUserById(String userId) {
		log.debug(String.format(
			"retrieving user with userId %s", userId));

		Object[] values = { userId };
		String query = String.format(
			"select * from %s where %s=?",
			TABLE_NAME, DBConstants.USER__ID);
		...
		try {
			...
			log.error(String.format(
				"getUserById user retrieve db error. userId=%s",
				userId), e);
//		} finally {
//			DBConnection.get().close(rs, null, conn);
		}
...
//			DBConnection.get().close(rs, null, conn);
	...
	public List<User> getUsersByClanId(String clanId) {
		log.debug(String.format(
			"retrieving users with clanId %s", clanId));

		Object[] values = { clanId };
		String query = String.format(
			"select * from %s where %s=?",
			TABLE_NAME, DBConstants.USER__CLAN_ID);
...
//			DBConnection.get().close(rs, null, conn);
	...
	public List<User> getUserByUDIDorFbId(String UDID, String fbId) {
		log.debug(String.format(
			"retrieving user with udid=%s, fbId=%s",
			UDID, fbId));

		Object[] values = { UDID, fbId };
		String query = String.format(
			"select * from %s where %s=? or %s=?",
			TABLE_NAME, DBConstants.USER__UDID, DBConstants.USER__FACEBOOK_ID);
		...
		try {
			...
			log.error(String.format(
				"user retrieve db error. UDID=%s, fbId=%s",
				UDID, fbId), e);
//		} finally {
//			DBConnection.get().close(rs, null, conn);
		}
...
//			}
		...
		public static List<String> getColumnsSelected() {
			if (null == columnsSelected) {
				...
				columnsSelected.add(DBConstants.USER__NUM_COINS_RETRIEVED_FROM_STRUCTS);
				columnsSelected.add(DBConstants.USER__NUM_OIL_RETRIEVED_FROM_STRUCTS);

				columnsSelected.add(DBConstants.USER__NUM_CONSECUTIVE_DAYS_PLAYED);
				...
				columnsSelected.add(DBConstants.USER__LAST_OBSTACLE_SPAWNED_TIME);
