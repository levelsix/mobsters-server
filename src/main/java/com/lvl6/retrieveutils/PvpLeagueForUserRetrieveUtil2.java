...
public class PvpLeagueForUserRetrieveUtil2 {
	private static Logger log = LoggerFactory.getLogger(PvpLeagueForUserRetrieveUtil2.class);
...
//			}
			...
			plfu = this.jdbcTemplate.queryForObject(query,
				values.toArray(), rowMapper);
		} catch (Exception e) {
			log.error(String.format(
				"could not retrieve user pvpLeague for userId=%s", userId), e);
		}
		
		return plfu;
	}
	
	public Map<String, PvpLeagueForUser> getUserPvpLeagueForUsers(List<String> userIdList) {
		...
		try {
			...
			log.error(String.format(
				"could not retrieve user pvpLeague for userIds=%s", userIdList), e);
		}
	...
	private static final class UserPvpLeagueForClientMapper implements RowMapper<PvpLeagueForUser> {
		...
		public PvpLeagueForUser mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			ofu.setUserId(rs.getString(DBConstants.PVP_LEAGUE_FOR_USER__USER_ID));
