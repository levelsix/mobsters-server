...
public class ClanRetrieveUtils2 {
	...
	public Clan getClanWithId(String clanId) {
		...
		try {
			...
			log.error(
					String.format("could not retrieve clan for id=%s", clanId),
					e);
		}
	...
	public Map<String, Clan> getClansByIds(Collection<String> clanIds) {
		...
		try {
			List<Clan> clans = this.jdbcTemplate.query(query, values.toArray(),
					rowMapper);
	...
	public List<Clan> getClansWithSimilarNameOrTag(String name, String tag) {
		log.debug(String.format("retrieving clan with name=%s, tag=%s", name,
				tag));

		String query = String.format(
				"SELECT * FROM %s WHERE %s LIKE ? OR %s LIKE ?", TABLE_NAME,
				DBConstants.CLANS__NAME, DBConstants.CLANS__TAG);
	...
	private static final class ClanForClientMapper implements RowMapper<Clan> {
		...
		public Clan mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			try {
				Timestamp time = rs
						.getTimestamp(DBConstants.CLANS__CREATE_TIME);
			...
			clan.setRequestToJoinRequired(rs
					.getBoolean(DBConstants.CLANS__REQUEST_TO_JOIN_REQUIRED));
		...
		public static List<String> getColumnsSelected() {
			if (null == columnsSelected) {
				...
				columnsSelected
						.add(DBConstants.CLANS__REQUEST_TO_JOIN_REQUIRED);
