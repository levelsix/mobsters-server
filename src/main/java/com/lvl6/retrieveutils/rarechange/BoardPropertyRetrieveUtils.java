...
@Component @DependsOn("gameServer") public class BoardPropertyRetrieveUtils {
	...
	public static BoardProperty getBoardPropertyForId(int boardPropertyId) {
		log.debug(String.format(
			"retrieve boardProperty data for boardProperty=%s", boardPropertyId));
		if (null == idsToBoardProperties) {
			setStaticIdsToBoardProperties();
		}
	...
	{
		log.debug(String.format(
			"retrieve boardProperty data for boardId=%s", boardId));
			...
			if (conn != null) {
				...
				if (rs != null) {
					try {
						rs.last();
						rs.beforeFirst();
						Map<Integer, BoardProperty> idsToBoardPropertiesTemp =
							new HashMap<Integer, BoardProperty>();
						
						Map<Integer, Collection<BoardProperty>> boardIdsToBoardPropertiesTemp =
							new HashMap<Integer, Collection<BoardProperty>>();
						...
						while(rs.next()) {  
							...
							idsToBoardPropertiesTemp.put(boardPropertyId, boardProperty);
							
							int boardId = boardProperty.getBoardId();
							if (!boardIdsToBoardPropertiesTemp.containsKey(boardId))
							{
								//base case
								boardIdsToBoardPropertiesTemp.put(boardId, new ArrayList<BoardProperty>());
							}
							
							Collection<BoardProperty> bpCollection =
								boardIdsToBoardPropertiesTemp.get(boardId);
	 ...
	 */
	private static BoardProperty convertRSRowToBoardProperty(ResultSet rs) throws SQLException {
