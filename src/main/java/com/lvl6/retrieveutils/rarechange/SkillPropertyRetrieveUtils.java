...
@Component @DependsOn("gameServer") public class SkillPropertyRetrieveUtils {
	...
	public static SkillProperty getSkillPropertyForSkillPropertyId(int skillPropertyId) {
		if (null == skillPropertyIdsToSkillPropertys) {
			setStaticSkillIdsToIdsToSkillPropertys();      
		}
		if (!skillPropertyIdsToSkillPropertys.containsKey(skillPropertyId)) {
			log.warn(String.format(
				"no skillProperty for skillPropertyId=%s", skillPropertyId));
			return null;
		}
		return skillPropertyIdsToSkillPropertys.get(skillPropertyId); 
	}

	public static Map<Integer, SkillProperty> getSkillPropertysForSkillId(int skillId) {
			...
			if (conn != null) {
				...
				if (rs != null) {
					try {
						rs.last();
						rs.beforeFirst();
						Map<Integer, Map<Integer, SkillProperty>> skillIdsToIdsToSkillPropertysTemp =
							new HashMap<Integer, Map<Integer, SkillProperty>>();
						Map<Integer, SkillProperty> skillPropertyIdsToSkillPropertysTemp =
						...
						while(rs.next()) {  
							...
							if (!skillIdsToIdsToSkillPropertysTemp.containsKey(skillId)) {
								skillIdsToIdsToSkillPropertysTemp.put(skillId, new HashMap<Integer, SkillProperty>());
							}
							...
							Map<Integer, SkillProperty> idsToSkillPropertys =
								skillIdsToIdsToSkillPropertysTemp.get(skillId);

							int skillPropertyId = skillProperty.getId();
							idsToSkillPropertys.put(skillPropertyId, skillProperty);
							skillPropertyIdsToSkillPropertysTemp.put(skillPropertyId, skillProperty);
						}
	 ...
	 */
	private static SkillProperty convertRSRowToSkillProperty(ResultSet rs) throws SQLException {
