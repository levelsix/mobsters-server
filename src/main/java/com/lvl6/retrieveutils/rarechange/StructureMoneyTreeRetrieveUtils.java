...
public class StructureMoneyTreeRetrieveUtils {
	...
	private static StructureMoneyTree convertRSRowToMoneyTree(ResultSet rs)
		...
		float productionRate = rs
				.getFloat(DBConstants.STRUCTURE_MONEY_TREE__PRODUCTION_RATE);
		;
		int capacity = rs.getInt(DBConstants.STRUCTURE_MONEY_TREE__CAPACITY);
		;
		int daysOfDuration = rs
				.getInt(DBConstants.STRUCTURE_MONEY_TREE__DAYS_OF_DURATION);
		;
		int daysForRenewal = rs
				.getInt(DBConstants.STRUCTURE_MONEY_TREE__DAYS_FOR_RENEWAL);
		;
		String iapProductId = rs
				.getString(DBConstants.STRUCTURE_MONEY_TREE__IAP_PRODUCT_ID);
		;
		String entireIAPProductId = Globals.APPLE_BUNDLE_ID() + "."
				+ iapProductId;
		String fakeIAPProductId = rs
				.getString(DBConstants.STRUCTURE_MONEY_TREE__FAKE_IAP_PRODUCT_ID);
		;
		...
		StructureMoneyTree smt = new StructureMoneyTree(structId,
				...
				entireIAPProductId, entireFakeIAPProductId);
