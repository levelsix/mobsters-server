...
public class BoosterItemRetrieveUtils {
	...
	private static Map<Integer, Map<Integer, BoosterItem>> boosterItemIdsToBoosterItemsForBoosterPackIds;
	...
	public void setStaticBoosterItemIdsToBoosterItemsForBoosterPackIds() {
		try {
			...
			boosterItemIdsToBoosterItemsForBoosterPackIds = new HashMap<Integer, Map<Integer, BoosterItem>>();
			List<BoosterItem> bis = new ArrayList<BoosterItem>(
					boosterItemIdsToBoosterItems.values());
			for (BoosterItem bi : bis) {
				int packId = bi.getBoosterPackId();
				if (!boosterItemIdsToBoosterItemsForBoosterPackIds
						.containsKey(packId)) {
					Map<Integer, BoosterItem> bItemIdToBItem = new HashMap<Integer, BoosterItem>();
					boosterItemIdsToBoosterItemsForBoosterPackIds.put(packId,
							bItemIdToBItem);
				}
				//each itemId is unique (autoincrementing in the table)
				Map<Integer, BoosterItem> itemIdToItem = boosterItemIdsToBoosterItemsForBoosterPackIds
						.get(packId);
				...
				if (rs != null) {
					try {
						...
						while (rs.next()) {  //should only be one
							...
							if (boosterItem != null)
								boosterItemIdsToBoosterItemsTemp.put(
	...
	private BoosterItem convertRSRowToBoosterItem(ResultSet rs)
		...
		boosterItem.setId(id);
		boosterItem.setBoosterPackId(boosterPackId);
		boosterItem.setChanceToAppear(chanceToAppear);
		boosterItem.setRewardId(rewardId);
		boosterItem.setSpecial(isSpecial);
