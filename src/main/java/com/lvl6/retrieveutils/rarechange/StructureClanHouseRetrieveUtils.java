...
@Component @DependsOn("gameServer") public class StructureClanHouseRetrieveUtils {
  ...
  public static StructureClanHouse getUpgradedClanHouseForStructId(int structId) {
  	...
  	if (null != curStruct) {
  		...
  		StructureClanHouse upgradedStruct = structIdsToClanHouses.get(successorStructId);
  		return upgradedStruct;
  	}
  	return null;
  }
  
  public static StructureClanHouse getPredecessorClanHouseForStructId(int structId) {
  	...
  	if (null != curStruct) {
  		...
  		StructureClanHouse predecessorStruct = structIdsToClanHouses.get(predecessorStructId);
  ...
  private static StructureClanHouse convertRSRowToClanHouse(ResultSet rs) throws SQLException {
    ...
    int maxHelpersPerSolicitation = rs.getInt(DBConstants.STRUCTURE_CLAN_HOUSE__MAX_HELPERS_PER_SOLICITATION);
    int teamDonationPowerLimit = rs.getInt(DBConstants.STRUCTURE_CLAN_HOUSE__TEAM_DONATION_POWER_LIMIT);
