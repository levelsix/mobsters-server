...
public class ClanRaidStageRetrieveUtils {
	...
	public static int getClanRaidStageHealthForCrsId(int crsId) {
		...
		if (clanRaidStageIdsToClanRaidStages.containsKey(crsId)) {
			...
			log.error("clan raid stage does not exist. clanRaidStageId="
					+ crsId);
			return 0;
		}
	...
	public static Map<Integer, ClanRaidStage> getClanRaidStagesForClanRaidId(
		...
		if (clanRaidIdsToClanRaidStageIdsToClanRaidStages
				.containsKey(clanRaidId)) {
			return clanRaidIdsToClanRaidStageIdsToClanRaidStages
					.get(clanRaidId);
		} else {
	...
	public static ClanRaidStage getFirstStageForClanRaid(int clanRaidId) {
		...
		Map<Integer, ClanRaidStage> stageNumsToStages = clanRaidIdsToStageNumToClanRaidStages
				.get(clanRaidId);
		...
		List<Integer> stageNumsAsc = new ArrayList<Integer>(
				stageNumsToStages.keySet());
	...
	public static ClanRaidStage getNextStageForClanRaidStageId(
		...
		if (!clanRaidStageIdsToClanRaidStages.containsKey(clanRaidStageId)) {
			log.error("no clan raid stage exists for clanRaidStageId="
					+ clanRaidStageId);
			return null;
		}
		ClanRaidStage curCrs = clanRaidStageIdsToClanRaidStages
				.get(clanRaidStageId);
			...
			if (conn != null) {
				...
				if (rs != null) {
					try {
						rs.last();
						rs.beforeFirst();
						Map<Integer, Map<Integer, ClanRaidStage>> clanRaidIdsToClanRaidStageIdsToClanRaidStagesTemp = new HashMap<Integer, Map<Integer, ClanRaidStage>>();
						Map<Integer, Map<Integer, ClanRaidStage>> clanRaidIdsToStageNumToClanRaidStagesTemp = new HashMap<Integer, Map<Integer, ClanRaidStage>>();
						Map<Integer, ClanRaidStage> clanRaidStageIdsToClanRaidStagesTemp = new HashMap<Integer, ClanRaidStage>();
						...
						while (rs.next()) {
							...
							if (!clanRaidIdsToClanRaidStageIdsToClanRaidStagesTemp
									.containsKey(clanRaidId)) {
								clanRaidIdsToClanRaidStageIdsToClanRaidStagesTemp
										.put(clanRaidId,
												new HashMap<Integer, ClanRaidStage>());
								clanRaidIdsToStageNumToClanRaidStagesTemp.put(
										clanRaidId,
										new HashMap<Integer, ClanRaidStage>());
							}
							...
							Map<Integer, ClanRaidStage> clanRaidStageIdsToClanRaidStagesForClanRaidStage = clanRaidIdsToClanRaidStageIdsToClanRaidStagesTemp
									.get(clanRaidId);

							int clanRaidStageId = clanRaidStage.getId();
							clanRaidStageIdsToClanRaidStagesForClanRaidStage
									.put(clanRaidStageId, clanRaidStage);
							clanRaidStageIdsToClanRaidStagesTemp.put(
									clanRaidStageId, clanRaidStage);

							//enabling getting stages in order
							Map<Integer, ClanRaidStage> stageNumToClanRaidStages = clanRaidIdsToStageNumToClanRaidStagesTemp
									.get(clanRaidId);
							int stageNum = clanRaidStage.getStageNum();
							stageNumToClanRaidStages.put(stageNum,
									clanRaidStage);
						}
	...
	private static ClanRaidStage convertRSRowToClanRaidStage(ResultSet rs)
		...
		ClanRaidStage clanRaidStage = new ClanRaidStage(id, clanRaidId,
				eventDuration, stageNum, name);
