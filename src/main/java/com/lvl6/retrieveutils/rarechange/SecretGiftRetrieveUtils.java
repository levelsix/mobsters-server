...
public class SecretGiftRetrieveUtils {
	...
	private static Map<Integer, SecretGiftConfig> idsToSecretGifts;
...
//	private static final String TABLE_NAME = DBConstants.TABLE_SECRET_GIFT_CONFIG;
	...
	public Map<Integer, SecretGiftConfig> getIdsToSecretGifts() {
	...
	public SecretGiftConfig getSecretGiftForId(int secretGiftId) {
		if (null == idsToSecretGifts) {
			reload();
		}
	...
	public Map<Integer, SecretGiftConfig> getsForIds(Collection<Integer> ids) {
		if (null == idsToSecretGifts) {
//			setStaticIdsTos();
			reload();
		}
		Map<Integer, SecretGiftConfig> returnMap = new HashMap<Integer, SecretGiftConfig>();

		for (int id : ids) {
			SecretGiftConfig sgc = getSecretGiftForId(id);
			returnMap.put(id, sgc);
		}
		return returnMap;
	}

	public SecretGiftConfig nextSecretGift(double probability) {
	...
	private void setStaticIdsToSecretGifts() {
		...
		Map<Integer, SecretGiftConfig> idsToSecretGiftTemp =
				new HashMap<Integer, SecretGiftConfig>();
		double secretGiftProbabilitySumTemp = 0D;

		try {
			//loop through each row and convert it into a java object
			for (SecretGiftConfig sgc : secretGiftConfigDao.findAll()) {
				int id = sgc.getId();
	...
	private void setUpRandomSecretGiftSelection() {
		...
		for (Integer id : ids) {
			SecretGiftConfig sgc = idsToSecretGifts.get(id);
