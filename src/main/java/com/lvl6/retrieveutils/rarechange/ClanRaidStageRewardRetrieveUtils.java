...
@Component @DependsOn("gameServer") public class ClanRaidStageRewardRetrieveUtils {
  ...
  public static ClanRaidStageReward getClanRaidStageRewardForClanRaidStageRewardId(int clanRaidStageRewardId) {
	  ...
	  if (idsToRewards.containsKey(clanRaidStageRewardId)) {
	  	...
	  	log.error("no reward for clanRaidStageRewardId=" + clanRaidStageRewardId);
	  	return null;
	  }
  }

  public static Map<Integer, ClanRaidStageReward> getClanRaidStageRewardsForClanRaidStageId(int clanRaidStageId) {
    log.debug("retrieve clan raid stage reward data for clanRaidStageId=" + clanRaidStageId);
    if (clanRaidStageIdsToIdsToRewards == null) {
      setStaticClanRaidIdsToClanRaidStageRewardIdsToClanRaidStageRewards();      
    }
    ...
    if (clanRaidStageIdsToIdsToRewards.containsKey(clanRaidStageId)) {
    	...
    	log.error("no clan raid stage rewards for clan raid stage id=" + clanRaidStageId);
			...
			if (conn != null) {
			  ...
			  if (rs != null) {
			    try {
			      rs.last();
			      rs.beforeFirst();
			      Map<Integer, Map<Integer, ClanRaidStageReward>> clanRaidIdsToClanRaidStageRewardIdsToClanRaidStageRewardsTemp =
			          new HashMap<Integer, Map<Integer, ClanRaidStageReward>>();
			      Map<Integer, ClanRaidStageReward> clanRaidStageRewardIdsToClanRaidStageRewardsTemp =
			      ...
			      while(rs.next()) {  
			        ...
			        if (!clanRaidIdsToClanRaidStageRewardIdsToClanRaidStageRewardsTemp.containsKey(clanRaidStageId)) {
			          clanRaidIdsToClanRaidStageRewardIdsToClanRaidStageRewardsTemp.put(clanRaidStageId, new HashMap<Integer, ClanRaidStageReward>());
			        }
			        ...
			        Map<Integer, ClanRaidStageReward> clanRaidStageRewardIdsToClanRaidStageRewardsForClanRaidStageReward =
			            clanRaidIdsToClanRaidStageRewardIdsToClanRaidStageRewardsTemp.get(clanRaidStageId);
			        
			        int clanRaidStageRewardId = clanRaidStageReward.getId();
			        clanRaidStageRewardIdsToClanRaidStageRewardsForClanRaidStageReward.put(clanRaidStageRewardId, clanRaidStageReward);
			        clanRaidStageRewardIdsToClanRaidStageRewardsTemp.put(clanRaidStageRewardId, clanRaidStageReward);
			      }
   ...
   */
  private static ClanRaidStageReward convertRSRowToClanRaidStageReward(ResultSet rs) throws SQLException {
    int i = 1;
    int id = rs.getInt(i++);
    ...
    ClanRaidStageReward clanRaidStageReward = new ClanRaidStageReward(id,
    		clanRaidStageId, minOilReward, maxOilReward, minCashReward, maxCashReward,
    		monsterId, expectedMonsterRewardQuantity);
