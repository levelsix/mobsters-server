...
//	private static final String TABLE_NAME = DBConstants.TABLE_GIFT_REWARD_CONFIG;
	private static Map<Integer, GiftRewardConfig> idsToGiftRewards;
	...
	public Map<Integer, GiftRewardConfig> getIdsToGiftRewards() {
	...
	public Collection<GiftRewardConfig> getGiftRewardForGiftId(int giftId) {
		...
		if (!giftIdToGiftRewards.containsKey(giftId)) {
			log.warn("no GiftRewards with giftId={}", giftId);
			return new ArrayList<GiftRewardConfig>();
		}
		...
		Collection<GiftRewardConfig> rewards = new ArrayList<GiftRewardConfig>();
		for (Integer id : idToReward.keySet())
		{
			GiftRewardConfig grc = getGiftReward(id);
			if (null != grc) {
				rewards.add(grc);
			}
		}
	...
	public GiftRewardConfig getGiftReward(int id) {
	...
	public GiftRewardConfig nextGiftReward(int giftId, double probability) {
	...
	private void setStaticIdsToGifts() {
		...
		Map<Integer, GiftRewardConfig> idsToGiftRewardsTemp = new HashMap<Integer, GiftRewardConfig>();
		...
		try {
			for (GiftRewardConfig grc : giftRewardConfigDao.findAll()) {
				int id = grc.getId();
