...
public class EndDungeonController extends EventController {
	...
	protected void processRequestEvent(RequestEvent event) throws Exception {
		...
		EndDungeonResponseProto.Builder resBuilder = EndDungeonResponseProto
				.newBuilder();
		...
		try {
			...
			TaskForUserOngoing ut = taskForUserOngoingRetrieveUtil
					.getUserTaskForId(userTaskId);
			boolean legit = checkLegit(resBuilder, aUser, userId, userTaskId,
					ut);
			...
			if (legit) {
				...
				successful = writeChangesToDb(aUser, userId, ut, userWon,
						curTime, money, maxCash, maxOil, firstTimeUserWonTask,
						tme, oldUtc, newUtc);
	...
	private boolean checkLegit(Builder resBuilder, User u, String userId,
		...
		if (null == u) {
			log.error(String.format("unexpected error: user is null. user=%s",
					u));
			return false;
		}
		...
		if (null == ut) {
			log.error(String.format(
					"unexpected error: no user task for id userTaskId=%s",
					userTaskId));
			return false;
		}
	...
	private boolean writeChangesToDb(User u, String uId, TaskForUserOngoing ut,
			...
			int maxCash, int maxOil, boolean firstTimeUserWonTask,
			TaskMapElement tme, UserTaskCompleted oldUtc,
			UserTaskCompleted newUtc) {
		...
		log.info(
				"user before currency change. {}, newUtc={}, oldUtc={}, cashGained={}, oilGained={}",
		...
		if (1 != num) {
			log.error(String
					.format("error inserting into user_task_history. numInserted=%s. Attempting to undo shi",
							num));
			updateUser(u, -1 * expGained, -1 * cashGained, -1 * oilGained,
					clientTime);
			return false;
		}
	...
	private void calculateResourcesGained(User u, TaskForUserOngoing ut,
			int maxCash, int maxOil, int remainingCash, int remainingOil,
			UserTaskCompleted utc, List<Integer> cashGainedContainer,
	...
	private boolean updateUser(User u, int expGained, int cashGained,
			int oilGained, Timestamp clientTime) {
		int energyChange = 0;
		if (!u.updateRelativeCashOilExpTasksCompleted(expGained, cashGained,
				oilGained, 1, clientTime)) {
			String preface = "problem updating user stats.";
			String midface = "increased tasks completed by 1,";
			log.error(String
					.format("%s expGained=%s, cashGained=%s, oilGained=%s, %s energyChange=%s, clientTime=%s, user=%s",
							preface, expGained, cashGained, oilGained, midface,
							energyChange, clientTime, u));
			return false;
		}
		return true;
	}
	...
	private void recordStageHistory(List<TaskStageForUser> tsfuList,
		...
		Map<Integer, Integer> tsmIdToQuantity = new HashMap<Integer, Integer>();
		...
		log.info(String
				.format("num task stage history rows inserted: num=%s, taskStageForUser=%s",
						num, tsfuList));

		//DELETE FROM TASK_STAGE_FOR_USER
		num = DeleteUtils.get().deleteTaskStagesForUserWithIds(userTaskStageId);
		log.info(String.format("num task stage for user rows deleted: num=%s",
				num));
		...
		for (int tsmId : tsmIds) {
			TaskStageMonster monsterThatDropped = monstersThatDropped
					.get(tsmId);
			...
			if (monsterDropLvl > 0) {
				...
				if (!monsterIdToLvlToQuantity.containsKey(monsterId)) {
					monsterIdToLvlToQuantity.put(monsterId,
			...
			if (monsterIdToNumPieces.containsKey(monsterId)) {
				int newAmount = numPiecesDroppedForMonster
	...
	private void awardOneTimeItem(Builder resBuilder, String userId,
			int itemId, int taskId, TaskMapElement tme) {
		log.info(String.format(
		...
		int quantity = 1;
		int numInserted = InsertUtils.get().insertIntoUpdateUserItem(userId,
				itemId, quantity);
		log.info(String.format("numInserted into userItem: %s", numInserted));
		if (numInserted > 0) {
			...
			if (ifuList.size() > 0) {
				...
				log.error(String.format(
						"unable to retrieve item for itemId=%s, userId=%s",
						itemId, userId));
			}
		} else {
			log.error(String.format(
					"unable to award item specified in TaskMapElement=%s", tme));
		}
	...
	*/
	...
	private void writeToTaskForUserCompleted(boolean userWon,
		...
		if (firstTimeUserWonTask) {
			...
			log.info(String
					.format("numInserted into task_for_user_completed: %s",
							numInserted));
			return;
		}
