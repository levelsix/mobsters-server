...
//      Monster aBoss = MonsterRetrieveUtils.getMonsterForMonsterId(bossId);
...
//        List<MonsterReward> brList = determineLoot(aUserBoss);
//
...
//   */
//  private boolean checkLegit(Builder resBuilder, User u, int userId, Monster b,
...
//   */
//  private boolean userHasSufficientEnergy(User u, Monster b,
...
//      Monster aBoss, Timestamp curTime, boolean isSuperAttack, List<Boolean> isCriticalHit) {
//    
...
//   */
//  private int generateDamage(Monster b, boolean isSuperAttack, User u,
...
//  }
//  
//  private List<MonsterReward> determineLoot(UserBoss aUserBoss) { 
//    List<MonsterReward> rewardsAwarded = new ArrayList<MonsterReward>();
//
//    if (aUserBoss.getCurrentHealth() <= 0) {
//      //get the BossRewards related to boss id
//      List<MonsterReward> brList = MonsterRewardRetrieveUtils.getAllBossRewardsForBossId(aUserBoss.getBossId());
//
//      Map<Integer, List<MonsterReward>> groupedBR = groupByRewardGroup(brList);
//
...
//
//
//  private Map<Integer, List<MonsterReward>> groupByRewardGroup(List<MonsterReward> brList) {
//    Map<Integer, List<MonsterReward>> groupedBRList = new HashMap<Integer, List<MonsterReward>>();
//    for(MonsterReward br : brList) {
//      int rewardGroup = br.getRewardGroup();
//      if(!groupedBRList.containsKey(rewardGroup)) {
//        groupedBRList.put(rewardGroup, new ArrayList<MonsterReward>());
//      }
...
//   */
//  private void pickLootFromSpecialRewardGroup(List<MonsterReward> brList,
//      List<MonsterReward> rewards) {
//    Random rand = new Random();
//    for(MonsterReward br: brList) {
...
//  }
//
//  private void pickLootFromRewardGroups(Map<Integer, List<MonsterReward>> groupedBR,
//      List<MonsterReward> rewards) {
//    for(List<MonsterReward> brList: groupedBR.values()) {
...
//        MonsterReward reward = fairlyPickReward(brList); 
//        rewards.add(reward);
//      } else {
//        MonsterReward reward = unfairlyPickReward(
...
//  }
//
//  private float sumProbabilities(List<MonsterReward> brList) {
...
//  private MonsterReward fairlyPickReward(List<MonsterReward> brList) {
...
//  private MonsterReward unfairlyPickReward(List<MonsterReward> brList, float highestProbability){
//    Random rand = new Random();
...
//      MonsterReward br = brList.get(index);
...
//  }
//
//  private void separateSilverGoldAndEquips(Builder resBuilder, List<MonsterReward> brList, List<Integer> allSilver,
//      List<Integer> allGold, List<Integer> allEquipIds, Map<String, Integer> money) {
//    int silverTotal = 0;
//    int goldTotal = 0;
//
//    for(MonsterReward reward: brList) {
...
//  }
//
//  private boolean isSilverReward(MonsterReward br) {
...
//  }
//
//  private boolean isGoldReward(MonsterReward br) {
...
//  }
//
//  private boolean isEquipReward(MonsterReward br) {
...
//      Monster aBoss, boolean isSuperAttack, Map<String, Integer> money,
