...
@Component @DependsOn("gameServer") public class EndClanAvengingController extends EventController {
	...
	protected void processRequestEvent(RequestEvent event) throws Exception {
		EndClanAvengingRequestProto reqProto = ((EndClanAvengingRequestEvent)event).getEndClanAvengingRequestProto();
		...
		EndClanAvengingResponseProto.Builder resBuilder = EndClanAvengingResponseProto.newBuilder();
	    ...
	    try {
	    	...
	    	for (String clanAvengeUuid : clanAvengeUuids)
	    	{
	    		try {
	    			...
	    			log.error("invalid UUID. not processing {}",
	    				clanAvengeUuid);
	    		}
	    	}
...
//	    }
	    ...
	    if (invalidUuids) {
	    	resBuilder.setStatus(EndClanAvengingStatus.FAIL_OTHER);
			EndClanAvengingResponseEvent resEvent = new EndClanAvengingResponseEvent(userId);
...
//	    locker.lockPlayer(userUuid, this.getClass().getSimpleName());
		try {
			EndClanAvengingAction bcaa =
				new EndClanAvengingAction(userId, clanId, clanAvengeUuids,
					DeleteUtils.get());
			bcaa.execute(resBuilder);


			EndClanAvengingResponseEvent resEvent =
				new EndClanAvengingResponseEvent(userId);
			resEvent.setTag(event.getTag());
			resEvent.setEndClanAvengingResponseProto(resBuilder.build());
