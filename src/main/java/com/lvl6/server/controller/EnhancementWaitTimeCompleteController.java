...
//				writeToUserCurrencyHistory(aUser, curTime, umcep.getUserMonsterId(), money, previousGems);
				writeToUserCurrencyHistory(aUser, curTime, curEnhancingMfuId, money, previousGems);
			}
		} catch (Exception e) {
	 ...
	 * 
	 * If user wants to speed up and if user does not have enough funds,
	 * will return fail.  
	 * 
	 * @param resBuilder
	 * @param u
	 ...
	 * @param inEnhancing (DEPRECATED) - the monsters that are in the enhancing queue
	 * @param umcep (DEPRECATED) - the base monster that is updated from using up some of the feeders
	 * @param usedUpUserMonsterIds (DEPRECATED) - userMonsterIds the user thinks has finished being enhanced
	 * @param speedUp
...
//					gemsForSpeedup, clientTime, umcep, userMonsterIds, u));
					...
					gemsForSpeedup, clientTime, u, curEnhancingMfuId));
				return false;
			} else {
				if (0 != gemCost) {
...
//			newExp, newLvl, newHp);
		...
		int num = UpdateUtils.get().updateCompleteEnhancing(uId, curEnhancingMfuId);
		log.info(String.format("numUpdated (enhancements completed, expected 1)=%s", num));
	...
	private void writeChangesToHistory(String uId,
		...
		log.info(String.format(
			"deleted monster healing rows. numDeleted=%s, userMonsterIds=%s, inEnhancing=%s",
		...
		log.info(String.format(
			"defeated monster_for_user rows. numDeleted=%s, inEnhancing=%s",
			num, inEnhancing));
