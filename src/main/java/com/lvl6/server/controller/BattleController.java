...
@Component @DependsOn("gameServer") public class BattleController extends EventController {
	...
	@Autowired
	protected UserStructRetrieveUtils userStructRetrieveUtils;
	...
	private int calculateLostCoins(User loser) {
		return -1*(int)(loser.getCoins()*0.033);
	}
 
	//robbing
	
//	public int calculateSilverStolenInBattle(User defender, Date clientDate) {
//		//get user strctures
//		Map<Integer, List<UserStruct>> usMap = getUserStructRetrieveUtils().getStructIdsToUserStructsForUser(defender.getId());
//		int buildingAmount = 0;  //save uncollected income here
//		
//		for (int structId : usMap.keySet()) {
//			Structure s = getStructureRetrieveUtils().getStructForStructId(structId);
//			List<UserStruct> usList = usMap.get(structId);
//			
//			//loop through the user structures with the specific structid to check if they've been collected yet
//			for(UserStruct us : usList) {
//				long timeIncomeBecameAvailable = clientDate.getTime() - s.getMinutesToGain()*60000;
//				if(us.getLastRetrieved().getTime() < timeIncomeBecameAvailable) {
//					
//					//wasn't collected, total amount susceptible to be stolen
//					buildingAmount += s.getIncome();
//				}
//			}
//		}
//
...
//	}
	...
	public UserStructRetrieveUtils getUserStructRetrieveUtils() {
		return userStructRetrieveUtils;
	}

	public void setUserStructRetrieveUtils(
			UserStructRetrieveUtils userStructRetrieveUtils) {
