...
public class SpawnObstacleController extends EventController{
	...
	protected void processRequestEvent(RequestEvent event) throws Exception {
		SpawnObstacleRequestProto reqProto = ((SpawnObstacleRequestEvent)event).getSpawnObstacleRequestProto();
		...
		SpawnObstacleResponseProto.Builder resBuilder = SpawnObstacleResponseProto.newBuilder();
		...
		try {
			...
			if (legitComplete) {
				success = writeChangesToDB(user, userId, clientTime, mopList, ofuList);
			}
			
			if (success) {
				//client needs the object protos but with the ids set
				for (ObstacleForUser ofu: ofuList) {
					UserObstacleProto obp = CreateInfoProtoUtils.createUserObstacleProto(ofu);
	...
	private boolean checkLegit(Builder resBuilder, int userId, User user,
		...
		if (null == user) {
			log.error("unexpected error: user is null. user=" + user + "\t userId=" + userId);
			return false;
		}
		
		if (null == mopList || mopList.isEmpty()) {
			log.error("no obstacles to spawn for user. mopList=" + mopList + "\t user=" + user);
			return false;
		}
