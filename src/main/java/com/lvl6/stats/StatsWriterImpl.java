	...
	protected void stats() {
		...
		try {
			for (String stat : stats.values.keySet()) {
				try {
					Serie serie = new Serie.Builder(businessMetricsSeriesBase
							+ stat).columns("value")
							.values(stats.values.get(stat)).build();
					metricsReporter.getInfluxdbBusiness().write(
							metricsReporter.getInfluxdbBusinessName(),
							TimeUnit.SECONDS, serie);
				} catch (Throwable t) {
