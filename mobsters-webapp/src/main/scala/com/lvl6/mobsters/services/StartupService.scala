...
class StartupService extends LazyLogging {
  ...
  def startup(event: RequestEvent, responses:ToClientEvents) = {
    ...
    if (updateStatus != UpdateStatus.MAJOR_UPDATE) {
      ...
      responses.preDBResponseEvents.add(PreDBResponseEvent(resEvent, udid))
    }
  }
...
//        scga.execute(fillMe);
  ...
  def forceLogoutOthers(udid: String, playerId: String, user: User, fbId: String, responses: ToClientEvents) = {
    ...
    if (fbId != null && !fbId.isEmpty()) {
      responses.preDBFacebookEvents.add(new PreDBFacebookEvent(fbId, logoutEvent))
    }
  }
