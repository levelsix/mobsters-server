...
@Component@Scope("prototype")public class AcceptOrRejectClanInviteAction {
	...
	public AcceptOrRejectClanInviteAction(String acceptedInviteId,
			...
			Date clientTime, List<String> rejectedInviteIds,
	...
	private boolean verifySyntax(Builder resBuilder) {
		...
		if (acceptedInviteId.isEmpty() && rejectedInviteIds.isEmpty()) {
			log.error(String
					.format("invalid request: did not accept or reject invites. acceptedInviteId=%s, rejectedInviteIds=%s",
							acceptedInviteId, rejectedInviteIds));
			return false;
		}

		if (inviterId.isEmpty() || clanId.isEmpty()) {
			log.error(String.format("invalid inviterId=%s or clanId=%s",
					inviterId, clanId));
			return false;
		}
	...
	private boolean verifySemantics(Builder resBuilder) {
		...
		if (null == prospectiveMember) {
			log.error(String.format("No user. invalid prospectiveMemberId=%s",
					prospectiveMemberId));
			return false;
		}
		...
		if (!prospectiveMember.getClanId().isEmpty()) {
			log.error(String.format("prospectiveMember already in clan. %s",
					prospectiveMember));
			return false;
		}
		...
		if (null == prospectiveClan) {
			log.error(String.format("No clan. invalid clanId=%s", clanId));
			return false;
		}
	...
	private boolean writeChangesToDB(Builder resBuilder) {
		...
		if (rejectOnly) {
			...
			int numDeleted = deleteUtil.deleteClanInvite(prospectiveMemberId,
					rejectedInviteIds);
			log.info(String.format("deleting invites. numDeleted=%s",
					numDeleted));
			...
			if (!prospectiveMember.updateRelativeCoinsAbsoluteClan(0, clanId)) {
				log.error(String
						.format("problem updating user to new clanId. user=%s, clanId=%s",
			...
			if (!insertUtil.insertUserClan(prospectiveMemberId, clanId,
					userClanStatus, new Timestamp(clientTime.getTime()))) {
