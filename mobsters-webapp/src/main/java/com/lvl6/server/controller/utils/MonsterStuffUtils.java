...
public class MonsterStuffUtils {
	...
	public List<String> getUserMonsterIds(
			List<FullUserMonsterProto> mfuList) {
		List<String> idList = new ArrayList<String>();
	...
	public Map<String, UserMonsterHealingProto> convertIntoUserMonsterIdToUmhpProtoMap(
	...
	public Map<String, UserEnhancementItemProto> convertIntoUserMonsterIdToUeipProtoMap(
	 ...
	 */
	public void retainValidMonsters(Set<String> domain,
			Map<String, ?> selectedMonsters, boolean keepThingsInDomain,
			boolean keepThingsNotInDomain) {
		...
		for (String selectedId : selectedIds) {
			...
			log.warn("Not retaining. object=" + umhp + ", keepThingsInDomain="
	 ...
	 */
	public void retainValidMonsterIds(Set<String> existing,
			List<String> ids) {
		//  	ids.add(123456789L);
		...
		if (copyIds.size() != ids.size()) {
			...
			log.warn("client asked for some invalid ids. asked for ids="
					+ copyIds + "\t existingIds=" + existing
	...
	public List<MonsterEnhancingForUser> convertToMonsterEnhancingForUser(
		...
		for (UserEnhancementItemProto ueip : protos.values()) {
			...
			MonsterEnhancingForUser mefu = new MonsterEnhancingForUser(userId,
					monsterForUserId, expectedStartTime, enhancingCost,
					enhancingFinished);//, queuedTime);
			nonProtos.add(mefu);
		}
	...
	public Map<Integer, Integer> completeMonstersFromQuantities(
			Map<Integer, MonsterForUser> monsterIdToIncompleteUserMonster,
			Map<Integer, Integer> monsterIdToQuantity, Date combineTime) {
		...
		Set<Integer> incompleteMonsterIds = new HashSet<Integer>(
				monsterIdToIncompleteUserMonster.keySet());
		...
		for (int monsterId : incompleteMonsterIds) {
			MonsterForUser mfu = monsterIdToIncompleteUserMonster
					.get(monsterId);
			...
			if (mfu.getNumPieces() >= monzter.getNumPuzzlePieces()) {
				log.warn("(will not process this one) somehow retrieved monster"
						+ " with max pieces, when it shouldn't be at max pieces. mfu="
						+ mfu);
				monsterIdToIncompleteUserMonster.remove(monsterId);

			} else {
				newPiecesAvailable = completePieceDeficientMonster(mfu,
						numPiecesAvailable, monzter, combineTime);
			}
	...
	public int completePieceDeficientMonster(MonsterForUser mfu,
			int numPiecesAvailable, Monster monzter, Date combineTime) {
	...
	public List<MonsterForUser> createMonstersForUserFromQuantities(
		...
		if (null != monsterIdToLvlToQuantity
				&& !monsterIdToLvlToQuantity.isEmpty()) {
		...
		if (!monsterIdsToQuantities.isEmpty()) {
			...
			for (int monsterId : monsterIds) {
				...
				log.info(String
						.format("for monsterId=%s and for quantity=%s, creating some number of a specific monster for a user. monster=%s",
								monsterId, quantity, monzter));

				List<MonsterForUser> newUserMonsters = createMonsterForUserFromQuantity(
						userId, monzter, quantity, combineStartTime, monsterLevelInfoRetrieveUtils);
				log.info(String
						.format("some amount of a certain monster created. monster(s)=%s",
								newUserMonsters));
		...
		if (null != monsterIdToLvlToQuantity
			...
			for (int monsterId : monsterIdToLvlToQuantity.keySet()) {
				...
				Map<Integer, Integer> lvlToQuantity = monsterIdToLvlToQuantity
						.get(monsterId);
				for (int lvl : lvlToQuantity.keySet()) {

					List<MonsterForUser> newUserMonsters = createLeveledMonsterForUserFromQuantity(
							userId, monzter, lvlToQuantity.get(lvl),
							combineStartTime, lvl, monsterLevelInfoRetrieveUtils);

					log.info(String
							.format("some amount of a certain leveled monster created. monster(s)=%s",
									newUserMonsters));
	...
	public List<MonsterForUser> createMonsterForUserFromQuantity(
		...
		for (; quantity > 0; quantity -= numPiecesForCompletion) {
			...
			MonsterForUser mfu = new MonsterForUser(id, userId, monsterId,
					currentExp, currentLvl, currentHealth, numPieces,
					hasAllPieces, isComplete, combineStartTime, teamSlotNum,
					sourceOfPieces, false, monzter.getBaseOffensiveSkillId(),
	...
	public List<MonsterForUser> createLeveledMonsterForUserFromQuantity(
		...
		for (; quantity > 0; quantity -= 1) {
			...
			MonsterForUser mfu = new MonsterForUser(id, userId, monsterId,
					currentExp, currentLvl, currentHealth, numPieces,
					hasAllPieces, isComplete, combineStartTime, teamSlotNum,
					sourceOfPieces, restricted,
					monzter.getBaseOffensiveSkillId(),
					monzter.getBaseDefensiveSkillId());
			returnList.add(mfu);
		}
	...
	public List<FullUserMonsterProto> updateUserMonsters(String userId,
		...
		log.info(String.format("the monsters the user gets:%s",
				monsterIdToLvlToQuantity));

		if ((null == monsterIdToNumPieces || monsterIdToNumPieces.isEmpty())
				&& (null == monsterIdToLvlToQuantity || monsterIdToLvlToQuantity
						.isEmpty())) {
			return new ArrayList<FullUserMonsterProto>();
		}

		//holds return values
		Map<Integer, MonsterForUser> monsterIdsToIncompletes = new HashMap<Integer, MonsterForUser>();
		Map<Integer, Integer> monsterIdToRemainingPieces = new HashMap<Integer, Integer>();
		awardPieces(userId, monsterIdToNumPieces, monsterIdsToIncompletes,
				monsterIdToRemainingPieces, combineStartDate);

		//UPDATE THESE INCOMPLETE MONSTERS, IF ANY. SINCE UPDATING, UPDATE THE
		//combineStartTime
		List<MonsterForUser> dirtyMonsterForUserList = new ArrayList<MonsterForUser>(
				monsterIdsToIncompletes.values());
		if (!dirtyMonsterForUserList.isEmpty()) {
			log.info("the monsters that are updated: {}",
					dirtyMonsterForUserList);
			UpdateUtils.get().updateUserMonsterNumPieces(userId,
					dirtyMonsterForUserList, sourceOfPieces, combineStartDate);
		}
		log.info("new monsters (in pieces) the user gets {}",
		...
		List<MonsterForUser> newMonsters = createMonstersForUserFromQuantities(
				userId, monsterIdToRemainingPieces, monsterIdToLvlToQuantity,
				combineStartDate, monsterLevelInfoRetrieveUtils);
	...
	private void awardPieces(String userId,
			Map<Integer, Integer> monsterIdToNumPieces,
			Map<Integer, MonsterForUser> monsterIdsToIncompletes,
			Map<Integer, Integer> monsterIdToRemainingPieces, Date combineTime) {
		if (null == monsterIdToNumPieces || monsterIdToNumPieces.isEmpty()) {
			//no pieces to award;
			return;
		}
		...
		monsterIdsToIncompletes.putAll(RetrieveUtils
				...
				.getPieceDeficientIncompleteMonstersWithUserAndMonsterIds(
						userId, monsterIdToNumPieces.keySet()));
		...
		monsterIdToRemainingPieces.putAll(completeMonstersFromQuantities(
				monsterIdsToIncompletes, monsterIdToNumPieces, combineTime));
	}

	//returns user monster ids
	public List<String> getWholeButNotCombinedUserMonsters(
		...
		Map<Integer, Monster> idsToMonsters = monsterRetrieveUtils
				.getMonstersForMonsterIds(monsterIds);
	...
	public Map<String, Integer> convertToMonsterForUserIdToCashAmount(
	...
	public MonsterForUser createNewUserMonster(String userId,
			int numPieces, Monster monzter, Date now, boolean hasAllPieces,
			boolean isComplete, MonsterLevelInfoRetrieveUtils monsterLevelInfoRetrieveUtils) {
		...
		MonsterForUser mfu = new MonsterForUser(id, userId, monsterId,
				...
				isComplete, now, teamSlotNum, sourceOfPieces, false,
				monzter.getBaseOffensiveSkillId(),
				monzter.getBaseDefensiveSkillId());
	...
	public Set<String> getUserMonsterIdsUsedInEvolution(
		...
		if (null != mefu) {
			...
			for (MonsterEvolvingForUser mefyou : catalystIdToMonsterEvolutionForUser
					.values()) {
	...
	public List<String> getUserMonsterIdsInClanRaid(
	...
	public Element getMonsterElementForMonsterId(int monsterId) {
		...
		if (null != mon) {
			...
			try {
				...
				log.error("invalid monsterElement. id=" + monsterId
						+ " monster=" + mon);
			}
		}
		return null;
	...
	public boolean checkAllUserMonstersAreComplete(
	...
	public Map<String, Integer> calculatePvpDropIds(
		...
		for (MonsterForUser userMonster : userMonsters) {
			...
			Monster mon = null;
			if (dropped) {
				mon = monsterRetrieveUtils.getMonsterForMonsterId(monsterId);
			}
	...
	public MonsterSnapshotForUser javafyFullUserMonsterProto(
	...
	public MonsterSnapshotForUser javafyUserMonsterSnapshotProto(
