...
public class Locker {
	...
	public boolean lockHazelcastMap(String mapToLock) {
		...
		try {
			if (lockMap.tryLock(hzMapLockName(mapToLock), LOCK_WAIT_SECONDS,
					TimeUnit.SECONDS)) {
			...
			log.warn("1failed to aquire lock for " + hzMapLockName(mapToLock),
					e);
		}
	...
	public boolean lockPlayer(String playerId,String lockedByClass) {
		...
		throw new RuntimeException("2Unable to obtain lock after "
				+ LOCK_WAIT_SECONDS + " seconds");
	}

	public boolean lockPlayers(UUID playerId1, UUID playerId2,
	...
	public boolean lockFbId(String fbId) {
		...
		try {
			if (lockMap.tryLock(fbIdLockName(fbId), LOCK_WAIT_SECONDS,
					TimeUnit.SECONDS)) {
	...
	public boolean lockClan(UUID clanUuid) {
		...
		log.warn("2failed to aquire lock for " + clanLockName(clanId)
				+ " after " + LOCK_WAIT_SECONDS + " seconds");
		return false;
		// throw new
		// RuntimeException("Unable to obtain lock after "+LOCK_WAIT_SECONDS+" seconds");
	}
