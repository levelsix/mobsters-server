...
public class MetricsReporter {
	...
	public void setup() {
		try {
			...
			Object[] args = { influxdbAddress, 8086, influxdbName,
					influxdbUser, influxdbPassword };
			//			log.info("Setting up InfluxDB connection address:{} port:{} dbname: {} user: {} pass: {}", args);
			final Influxdb influxdb = new Influxdb(influxdbAddress, 8086,
					influxdbName, influxdbUser, influxdbPassword); // http transport
			final InfluxdbReporter reporter = InfluxdbReporter
					.forRegistry(registry)
					.prefixedWith(environment + "." + hostname)
					.convertRatesTo(TimeUnit.SECONDS)
					.convertDurationsTo(TimeUnit.MILLISECONDS)
					.filter(MetricFilter.ALL).skipIdleMetrics(false) // Only report metrics that have changed.
					.build(influxdb);
	...
	public void setupBusiness() {
		try {
			hostname = InetAddress.getLocalHost().getHostName();
			Object[] args = { influxdbAddress, 8086, influxdbBusinessName,
					influxdbUser, influxdbPassword };
			//			log.info("Setting up InfluxDB connection address:{} port:{} dbname: {} user: {} pass: {}", args);
			final InfluxDB influxDB = InfluxDBFactory.connect("http://"
					+ influxdbAddress + ":" + 8086, influxdbUser,
					influxdbPassword);//new Influxdb(influxdbAddress, 8086, influxdbBusinessName, influxdbUser, influxdbPassword); // http transport
